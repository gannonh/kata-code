...
- **Add project.** Type `Add project` and activate it. The palette stays open on Sources (`Local folder`). Combobox name on Sources is `Search...`. After `Local folder`, the combobox is named `Enter path (e.g. ~/projects/my-app)` and is prefilled with `~/`. One Escape closes the palette from any add-project view. Do not add a folder unless you are also running the landing recipe.
