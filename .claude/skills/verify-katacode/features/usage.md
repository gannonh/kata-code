...
- **Breakdown.** Choose `Hour` in `Usage breakdown`. The table headers are `Hour`, then one column per provider that has activity, `Total`, `Tokens`. An empty window is `Hour`, `Total`, `Tokens` only. Choose `Model`. The headers are `Model`, `Cost`, `Share`, `Tokens`.
