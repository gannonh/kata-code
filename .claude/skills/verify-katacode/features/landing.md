...
- **Hero entry.** Choose `Add project`. Run `agent-browser --session katacode-verify click` on the button named `Add project`. A dialog named `Command palette` appears with `data-palette-mode="command"`. The listbox under group `Sources` starts with `Local folder`, then `Git URL` and `GitHub repository`. Provider sources that are not ready show a `Setup Required` button instead of an enabled option. On a single environment this is the first step. Combobox name on Sources is `Search...`.
