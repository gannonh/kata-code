 ...
 *
 * `KATACODE_OTEL_SDK_DISABLED` is read first, so a machine that sets
 * `OTEL_SDK_DISABLED` for everything else can still opt Kata Code back in.
 *
...
/** The signals Kata Code exports, spelled as the variable names spell them. */
...
export const load: Effect.Effect<OtelEnvironment> = Config.all({
  t3: flag(
    ...
    T3CODE_TRUE,
    T3CODE_FALSE,
    (value) => `KATACODE_OTEL_SDK_DISABLED=${value} is not a yes or a no and was ignored`,
  ),
...
/**
 * Where one signal exports and how. `KATACODE_OTLP_*_URL` wins outright with
 * Kata Code's own export, then an OTEL endpoint with its own headers and
 * protocol, since `KATACODE_OTLP_HEADERS` was written for a different
 * collector, then the first of `fallbackUrls` with Kata Code's own export.
 */
