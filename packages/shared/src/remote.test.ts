...
describe("remote", () => {
  ...
  it("normalizes a protocol-relative host from a hosted pairing link", () => {
    expect(
      resolveRemotePairingTarget({
        pairingUrl: "https://app.kata.sh/pair?host=%2F%2Fremote.example.com#token=pairing-token",
      }),
    ).toEqual({
      credential: "pairing-token",
      httpBaseUrl: "https://remote.example.com/",
  ...
  it("rejects unsupported hosted pairing backend protocols", () => {
    ...
    try {
      resolveRemotePairingTarget({
        pairingUrl:
          "https://app.kata.sh/pair?host=ftp%3A%2F%2Fremote.example.com#token=pairing-token",
      });
    } catch (cause) {
      hostError = cause;
    }
