...
const SPEC_OFF =
  "OTEL_SDK_DISABLED is set, so no telemetry is exported, whatever configured it; set KATACODE_OTEL_SDK_DISABLED=false to export anyway";
const T3_OFF =
  "KATACODE_OTEL_SDK_DISABLED is set, so no telemetry is exported, whatever configured it";
const specIgnored = (value: string) =>
  `OTEL_SDK_DISABLED=${value} was read as false; the OpenTelemetry specification recognizes only the string true, so use OTEL_SDK_DISABLED=true or KATACODE_OTEL_SDK_DISABLED to say it any other way`;
...
describe("OtelEnvironment", () => {
  it.effect.each([
    ...
    // KATACODE_OTEL_SDK_DISABLED takes Config.Boolean's values, case-insensitively.
    { name: "t3 1", env: { KATACODE_OTEL_SDK_DISABLED: "1" }, disabled: true, warnings: [T3_OFF] },
    {
      name: "t3 TRUE",
      env: { KATACODE_OTEL_SDK_DISABLED: "TRUE" },
      disabled: true,
      warnings: [T3_OFF],
    },
    { name: "t3 n", env: { KATACODE_OTEL_SDK_DISABLED: "n" }, disabled: false, warnings: [] },
    {
      name: "t3 false overrides spec true",
      env: { KATACODE_OTEL_SDK_DISABLED: "false", OTEL_SDK_DISABLED: "true" },
      disabled: false,
      warnings: [],
    },
    {
      name: "blank t3 falls through",
      env: { KATACODE_OTEL_SDK_DISABLED: "  ", OTEL_SDK_DISABLED: "true" },
      disabled: true,
      warnings: [SPEC_OFF],
    },
    {
      name: "unreadable t3 warns and falls through",
      env: { KATACODE_OTEL_SDK_DISABLED: "maybe", OTEL_SDK_DISABLED: "true" },
      disabled: true,
      warnings: ["KATACODE_OTEL_SDK_DISABLED=maybe is not a yes or a no and was ignored", SPEC_OFF],
    },
    {
      name: "bad spec value still warns when t3 answered",
      env: { KATACODE_OTEL_SDK_DISABLED: "false", OTEL_SDK_DISABLED: "yes" },
      disabled: false,
      warnings: [specIgnored("yes")],
    },
  ...
  describe("endpoints", () => {
    ...
    it.effect.each([
      ...
      },
      {
        ...
        env: {
          KATACODE_OTEL_SDK_DISABLED: "true",
          OTEL_EXPORTER_OTLP_ENDPOINT: "https://collector:4318/base",
        },
        traces: "Unset",
        metrics: "Unset",
  ...
  describe("resolveSignalEndpoint", () => {
    ...
    it.each([
      {
        name: "KATACODE_OTLP_*_URL wins over an OTEL endpoint",
        otel: withLogs(otelExport),
        t3Url: "http://t3:4318/v1/logs",
        expected: { url: "http://t3:4318/v1/logs", export: t3Export },
      },
      {
        name: "KATACODE_OTLP_*_URL wins over a signal the OTEL variables turned off",
