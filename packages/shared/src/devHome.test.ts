...
import { resolveGitWorktreePath, resolveWorktreeKatacodeHome } from "./devHome.ts";
...
describe("resolveWorktreeKatacodeHome", () => {
  it.effect("answers with .katacode before the dev runner creates it", () =>
    Effect.gen(function* () {
      const { root, nested } = yield* makeRepo("worktree");
      const home = yield* resolveWorktreeKatacodeHome(nested);
      assert.equal(home, NodePath.join(NodePath.resolve(root), ".katacode"));
