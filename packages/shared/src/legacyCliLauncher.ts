/**
 * `dist/bin.mjs` of the `@kata-sh/code-cli` npm package: the entry point boot-service
 ...
 * executable in the sibling `@kata-sh/code-cli-<platform>-<arch>` package.
 *
...
const executable = join(dirname(require.resolve("@kata-sh/code-cli-" + process.platform + "-" + process.arch + "/package.json")), executableName);
...
const fail = (error) => {
  if (!error) return;
  process.stderr.write("katacode: " + error.message + "\\n");
  child.kill("SIGTERM");
  process.exitCode = 1;
};
if (ipc) {
