import { AuthStandardClientScopes, EnvironmentId } from "@kata-sh/code-contracts";
...
function pairingHttpLayer(
  ...
  const fetchFn = ((input, init = {}) => {
    ...
    if (url.endsWith("/.well-known/kata/environment")) {
...
describe("connection onboarding", () => {
  it.effect("prepares a persisted bearer registration from pairing details", () =>
    Effect.gen(function* () {
      ...
      expect(tokenParams.get("client_label")).toBe("Kata Code Test");
    }),
  );
