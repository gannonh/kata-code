    ...
    it.live.each(["npx", "npm"] as const)(
      ...
      (packageManager) =>
        Effect.gen(function* () {
          ...
          const packageSpec = "@kata-sh/code-cli@0.0.35";
  ...
  () => {
    ...
    it.live.each(cases)("handles $packageManager/$mode", ({ packageManager, mode }) =>
      Effect.gen(function* () {
        ...
        const packageSpec = "@kata-sh/code-cli@0.0.39-nightly.20260905.1286";
...
`,
        ...
        yield* fs.chmod(path.join(bin, packageManager), 0o700);
        if (mode === "existing-cli") yield* fs.symlink(cliPath, path.join(bin, "katacode"));
        ...
        if (installFailed) {
          ...
          assert.include(stderr, "npm produced no katacode executable");
