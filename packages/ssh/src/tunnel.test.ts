...
describe("ssh tunnel scripts", () => {
  it("installs and runs the release archive without Node, npm, or npx", () => {
    ...
    assert.include(
      script,
      "T3_RELEASE_BASE_URL='https://github.com/gannonh/kata-code/releases/download'",
    );
    assert.include(script, 'T3_RUNTIME_DIR="$HOME/.katacode/runtime/versions/$T3_ARCHIVE_VERSION"');
    assert.include(
      script,
      'T3_ARCHIVE="katacode-$T3_ARCHIVE_VERSION-$T3_PLATFORM-$T3_ARCH.tar.gz"',
    );
    assert.include(script, "SHA256SUMS");
    assert.include(script, 'exec "$T3_RUNTIME_DIR/katacode" "$@"');
    ...
    assert.isBelow(
      script.indexOf('"$T3_STAGING/katacode" --version'),
  ...
  it("builds the remote Kata Code runner with a node script override", () => {
  ...
  it("uses the remote Kata Code runner for launch and pairing scripts", () => {
    const target = {
      alias: "devbox",
    ...
    assert.include(launch, "Remote Kata Code server did not become ready");
...
describe("archive runner script", () => {
  ...
  const makeMirror = Effect.fn("makeMirror")(function* (root: string) {
    ...
    const stem = `katacode-${archiveVersion}-${platform}-${arch}`;
    const stage = `${root}/stage/${stem}`;
    const release = `${root}/mirror/v${archiveVersion}`;
    const script = [
      "set -eu",
      `mkdir -p '${stage}' '${release}'`,
      `printf '#!/bin/sh\\necho katacode v${archiveVersion}\\n' > '${stage}/katacode'`,
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const runner = `${root}/run-katacode.sh`;
        ...
        for (const result of results) {
          ...
          assert.include(result.stdout, `katacode v${archiveVersion}`);
        }
        const versionsDir = `${home}/.katacode/runtime/versions`;
