...
describe("ssh tunnel scripts", () => {
  ...
  it("shell-quotes package specs in the remote t3 runner", () => {
    ...
    assert.include(
      script,
      "require_installed_cli npm exec --yes --package '@kata-sh/code-cli@nightly; touch /tmp/t3-owned'",
    );
    assert.notInclude(script, "--package @kata-sh/code-cli@nightly; touch /tmp/t3-owned");
  });
