...
# so a failed build (the CLI depends on node-pty, which needs a C toolchain)
# leaves the npx cache without a katacode executable. \`npx --yes\` then exits 0
...
require_installed_cli() {
  T3_CLI_PATH="$("$@" -- sh -c 'command -v katacode' || true)"
  if [ -n "$T3_CLI_PATH" ]; then
    return 0
  fi
  printf 'Remote host installed %s but npm produced no katacode executable, which usually means a native dependency (node-pty) failed to build. Install a C toolchain on the remote host (Debian/Ubuntu: build-essential, Fedora/RHEL: gcc-c++ make, macOS: xcode-select --install) and try again.\\n' @@T3_PACKAGE_SPEC@@ >&2
  return 1
}
if command -v npx >/dev/null 2>&1; then
  require_installed_cli npx --yes --package @@T3_PACKAGE_SPEC@@ || exit 1
  exec npx --yes @@T3_PACKAGE_SPEC@@ "$@"
fi
if command -v npm >/dev/null 2>&1; then
  require_installed_cli npm exec --yes --package @@T3_PACKAGE_SPEC@@ || exit 1
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  if ! wait_ready "@@T3_READY_TIMEOUT_MS@@"; then
    printf 'Remote Kata server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
    if [ -s "$LOG_FILE" ]; then
...
export function buildRemoteT3RunnerScript(input?: RemoteT3RunnerOptions): string {
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "@kata-sh/code-cli@latest");
