...
import { cliReleaseDownloadBaseUrl } from "@kata-sh/code-shared/cliRelease";
...
if [ -z "$T3_ARCHIVE_VERSION" ]; then
  printf 'No Kata Code release version was provided for the remote runtime.\\n' >&2
  exit 1
fi
# Self-contained release archive: no Node, npm, or compiler on the remote.
# Unpacked into the pinned-runtime layout so \`katacode service install\` reuses it.
T3_RELEASE_BASE_URL=@@T3_RELEASE_BASE_URL@@
T3_RUNTIME_DIR="$HOME/.katacode/runtime/versions/$T3_ARCHIVE_VERSION"
t3_runtime_ready() {
  [ -x "$T3_RUNTIME_DIR/katacode" ] && [ "$(cat "$T3_RUNTIME_DIR/.install-complete" 2>/dev/null)" = "$T3_ARCHIVE_VERSION" ]
}
if ! t3_runtime_ready; then
  mkdir -p "$HOME/.katacode/runtime/versions"
  ...
  # under it.
  T3_LOCK="$HOME/.katacode/runtime/versions/.$T3_ARCHIVE_VERSION.install.lock"
  ...
  while ! mkdir "$T3_LOCK" 2>/dev/null; do
    ...
    if [ "$T3_LOCK_WAITED" -ge @@T3_ARCHIVE_LOCK_WAIT_SECONDS@@ ]; then
      printf 'Another Kata Code %s installation has held %s for too long.\\n' "$T3_ARCHIVE_VERSION" "$T3_LOCK" >&2
      exit 1
    fi
    sleep 1
...
if ! t3_runtime_ready; then
  case "$(uname -s)" in
    ...
    *) printf 'Remote host %s has no Kata Code release archive.\\n' "$(uname -s)" >&2; exit 1 ;;
  esac
  case "$(uname -m)" in
  ...
  T3_ARCHIVE="katacode-$T3_ARCHIVE_VERSION-$T3_PLATFORM-$T3_ARCH.tar.gz"
  T3_STAGING="$(mktemp -d "$HOME/.katacode/runtime/versions/.staging-XXXXXX")"
  ...
  if ! "$T3_STAGING/katacode" --version >/dev/null 2>&1; then
    printf 'The Kata Code %s executable does not run on this host.\\n' "$T3_ARCHIVE_VERSION" >&2; exit 1
  fi
...
RUNNER_FILE="$STATE_DIR/run-katacode.sh"
RUNNER_NEXT="$STATE_DIR/run-katacode.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  if ! wait_ready "@@T3_READY_TIMEOUT_MS@@"; then
    printf 'Remote Kata Code server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
    if [ -s "$LOG_FILE" ]; then
...
if [ "$REMOTE_MANAGED" != "external" ] && [ -n "$REMOTE_PID" ] && kill -0 "$REMOTE_PID" 2>/dev/null; then
  ...
  if kill -0 "$REMOTE_PID" 2>/dev/null; then
    printf 'Remote Kata Code server with PID %s did not stop within 2 seconds. Its ownership files were kept.\\n' "$REMOTE_PID" >&2
    exit 1
  fi
fi
rm -f "$PID_FILE" "$PORT_FILE" "$MANAGED_FILE"
...
export class SshInvalidArchiveVersionError extends Schema.TaggedError<SshInvalidArchiveVersionError>()(
  ...
  override get message(): string {
    return `'${this.archiveVersion}' is not an exact Kata Code version and cannot name a runtime directory.`;
  }
}
...
export class SshMissingRunnerError extends Schema.TaggedError<SshMissingRunnerError>()(
  ...
  override get message(): string {
    return "A remote Kata Code runner needs an archive version or a node script path.";
  }
}
