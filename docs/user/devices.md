...
`agent-device` command line, which Kata Code sets up for them.
...
Studio's Device Manager. Kata Code detects standard SDK locations; set
...
timeline. Agents drive the device through the `agent-device` command line. Kata
...
local network, Tailscale, and Kata Code Connect. Live video needs a secure page
...
Kata Code checks common Homebrew and Android SDK locations; custom installations need
...
sessions and stops its Kata Code helpers when reachable; simulators keep running.

Kata Code provides discovery, streaming, and control. Arrange app builds,
