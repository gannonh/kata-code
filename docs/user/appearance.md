...
On web and desktop, choose **Create theme** to adjust a palette, or import a Kata Code or VS Code
...
main local environment. [app.kata.sh](https://app.kata.sh) and additional connections do not use them.
...
takes precedence. If the server stops publishing the selected theme, Kata Code falls back to its
standard theme.

Run this on the server to set a default and switch connected clients to it:

```bash
katacode theme set nightfall
```

Clients that are offline apply it when they reconnect. Each client applies the setting once;
choosing another theme afterward sticks until the next `katacode theme set`. Run the command again to
reapply it, even if the name is unchanged.

`katacode theme clear` removes the default without changing anyone's current theme. `katacode theme show` lists
the default and published themes.

### Publish a theme

Save a theme exported from Kata Code into `~/.katacode/userdata/themes/` on the server, or the `themes`
directory under your custom state directory. The filename supplies the theme ID: `nightfall.json`
can be selected with `katacode theme set nightfall`. Keep the filename stable when updating its colors.
...
Set `appearance` to `light` or `dark` and supply hex colors for `canvas` and `accent`. Kata Code
