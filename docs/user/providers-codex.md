...
This guide is for people who want to use more than one Codex account in Kata Code. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install Kata Code](./install.md).
...
In Kata Code Settings, name it something obvious:
...
In Kata Code Settings, add another Codex provider:
...
Kata Code shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Kata Code treats it
