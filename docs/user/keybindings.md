...
`~/.katacode/userdata/keybindings.json` by default. You can edit this file directly.
...
Kata Code creates the file with its defaults and adds new defaults on later startups.
...
Invalid rules are ignored; if the file cannot be parsed, Kata Code uses defaults.
...
## Terminal, file search, and theme editor
