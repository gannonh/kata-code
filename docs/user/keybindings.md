...
The same configuration lives in `~/.katacode/userdata/keybindings.json` on the machine running the
server, if you prefer editing it directly. Kata Code writes the built-in defaults into that file on
