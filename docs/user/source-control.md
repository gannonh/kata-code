...
Kata Code integrates with GitHub, GitLab, Forgejo, Gitea, Bitbucket, and Azure DevOps to clone and publish
repositories, create pull requests, and review changes.

## Connect an account

Install Git and configure authentication on the machine running your Kata Code server. For a remote
...
[Gitea CLI (`tea`)](https://gitea.com/gitea/tea) 0.16 or later on your Kata Code server.
Sign in with `fj --host https://your-server auth add-token` or `tea login add`.
Repeat for each server you use, including Codeberg.

Kata Code prefers a matching `fj` login and falls back to `tea` when `fj` is unavailable
...
```bash
export KATACODE_BITBUCKET_ACCESS_TOKEN="your-access-token"
```
...
Use a thread's Git actions to commit, push, and create a pull request. Kata Code can generate commit
...
On GitHub these are GitHub's own viewed marks, so a review carries between Kata Code and github.com
in either direction. Forgejo, GitLab, Bitbucket, and Azure DevOps expose no record Kata Code can read, so the
server you are connected to keeps them instead: they follow you across the apps connected to that
server, but the host's own site will not show them, and the count reads **viewed in Kata Code**.
