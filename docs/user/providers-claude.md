...
This guide is for people who want to use more than one Claude setup in Kata Code. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install Kata Code](./install.md).
...
An empty `CLAUDE_CONFIG_DIR path` means Kata Code uses Claude Code's normal config directory.

When you set this field, Kata Code points Claude Code at that directory with the
...
Kata Code looks for Claude skills in the Claude config directory's `skills` folder, then
...
`~/.claude_personal_home/.claude`, which is not where Kata Code looks.
...
Kata Code only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so Kata Code keeps separate config directories isolated
