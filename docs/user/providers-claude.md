...
Kata Code uses Claude Code's login and configuration. Start with the default provider
...
Kata Code uses the Claude configuration on the connected server.
...
Select the model you want in Kata Code. For an OpenRouter model outside the built-in
...
do not replace the explicit model ID selected in Kata Code. Custom models may have
