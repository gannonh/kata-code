...
Kata Code shows a setup flow when you open a new installation or connect to the
hosted app for the first time. Existing workspaces skip this flow.

## Connect your computers

Select one or more computers to set up. If you opened Kata Code directly from a
...
If Kata Code cannot confirm the workspace during startup, the setup flow shows
**Still connecting** instead of opening the app. Select **Reload** to try again.

If Kata Code cannot read your saved settings, it shows **Could not read settings**.
...
Kata Code finds directories that Claude Code or Codex has used. Git repositories
...
A large or malformed history can reach the scan limit. Kata Code keeps the
...
30 days. You can continue those conversations in Kata Code.

Conversation import is best effort. Kata Code keeps the first user prompt and the
...
conversation so Kata Code does not remove user text. It reads one conversation at
