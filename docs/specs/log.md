...
## 2026-07-09 (Phase 3b lifecycle and identity recovery remediation)

Updated the Phase 3b documentation after the whole-branch strict-quality remediation. The [initial implementation plan](/specs/2026-07-04-kata-environments-deployments-phase-3b-plan.md) now points to the [durable lifecycle design](/specs/2026-07-07-kata-sandbox-lifecycle-design.md) for current lifecycle semantics. The [Phase 3b deep-dive](/specs/2026-07-04-kata-environments-deployments-phase-3b-design.md) distinguishes the historical snapshot/lapse/resume design from the implemented named persistent stop/start/delete model. The lifecycle record now covers probe isolation, runtime-configured session storage, Vercel name namespacing, per-instance operation ownership, provider-login cancellation, confirmed client-side disposal cleanup, and non-destructive `unknown` runtime state. Marked [sandbox identity recovery](/specs/2026-07-08-sandbox-identity-recovery-plan.md) Implemented and closed the related [deferred-work](/specs/deferred-work.md) entries for provider sign-in and durable session reclamation. Updated the [specs roadmap](/specs/index.md). Validation before this docs pass: `vp check`, `vp run typecheck`, `vp run test`, `vp run release:smoke`.

## 2026-07-08 (Environments / Deployments — Sandbox lifecycle rework implemented)

Marked the [Sandbox lifecycle — durable stop/start for Vercel and Docker](/specs/2026-07-07-kata-sandbox-lifecycle-design.md) spec `Implemented` (AC-L1–L15). Replaced the provision-once/dispose lifecycle with a durable create/stop/start/delete model: an optional SPI `lifecycle` capability (stop/start/status) on both drivers, Vercel v2 persistent sandboxes addressed by a deterministic name with `keepLastSnapshots {count:1}`, Docker named containers that stop/start without destroying the filesystem, a durable `SandboxSessionStore` at `~/.katacode/userdata/sandbox-sessions.json` with boot reconcile, `stopSession` RPC + unified `startSession` (provision or start-from-stopped), server-side `disposeSession`, removed `resumeSession`/`createSnapshot` + keepalive/lapse machinery, a state-driven Environments card (`Create & run sandbox`/`Stop`/`Start`/`Delete sandbox` with a status badge replacing the Enabled toggle), and Available Runtimes reconciliation (running/stopped/gone) that retro-fixes orphaned saved runtime records. No plaintext secrets in the store (Vercel auth trio + relay bearer token stripped). Build-blocking SDK spike resolved: `Sandbox.get({ name, resume: false })` returns a readable status for a stopped sandbox. Gates: `vp check`, `vp run typecheck`, `vp run test`, `vp run release:smoke` pass; `@environments-deploy` E2E updated (Docker stop/start automated; Vercel maintainer-local UAT). See the [build completion report](/specs/2026-07-07-kata-sandbox-lifecycle-build-report.md). Follow-ups: server-side `savedSandboxEnvironments` removal in dispose, `KATACODE_HOME` threading for the store, sandbox lifecycle event stream.
