...
- Take upstream's `DesktopUpdates.ts` support for a `deb` package type. It is inert until Kata ships a `.deb` (KAT-3473), and the Linux disabled-reason message keeps Kata's AppImage wording.
...
- Skip the new Apple `iphone-duo.glb` and the `sources.json` edit. `sources.json` records `"author": "Apple Inc."` and no redistribution license; KAT-3454 made the same SKIP and KAT-3455 holds the decision. `deviceModels.ts` still returns no model, so upstream's iPhone Duo 3D viewport and hinge controls stay inactive in Kata (`DeviceStreamView` renders them only when a Duo model exists). `docs/user/devices.md` drops the iPhone Duo sentences. Android foldables use a procedural scene and work.
...
- Changed Kata behavior, accepted as an adaptation: when the connector fails to start on a relay config save, upstream keeps the stored config so startup can retry instead of unwinding it. Kata previously removed the runtime config and the routine callback URL. The callback URL and `managedTunnelActive` now stay after a failed start. `managedCallbackReady` is true only while the stored URL exists and the connector reports `running`, the same rule `RoutineConnections` applies before registering webhooks. A tunnel started through the stored-config fallback therefore reads as ready without the relay-confirmed origin marker, and Settings shows no Repair prompt while it runs. `server.test.ts` records the sequence with a runtime mock whose status follows the applied config.
- `deploy-relay.yml` takes `RELAY_TUNNEL_CLEANUP_MODE` (default `off`, per the rollout doc). The release doc tells operators to rerun the workflow with `dry_run` unchecked where upstream says to force a deploy. It skips upstream's `push` trigger, the Blacksmith runner, the job-level `main` guard (it would block Kata's branch dry runs), and the `force` input. Kata changes the Worker revision on every deploy (`RELAY_DEPLOYMENT_REVISION`), and `infra/relay/scripts/deploy.test.ts` asserts no `deploy --force`.
...
An independent Opus reviewer read the complete fork delta. It ran a per-file three-way merge for all 530 upstream paths and compared the result with the candidate, then checked every upstream line missing from or kept in the candidate against a SKIP, rebrand, or Kata divergence. Identity, Connect wire identity, the Linear OAuth broker, callback URL handling, migrations, workflows, conflict resolutions, retained process and sandbox behavior, deliberate SKIPs, and the 44 trusted files were clean, apart from the disclosed `ProviderInstanceRegistryLive.test.ts`. It found four defects on `9df578c7c5`, all fixed in the next commit: the relay runbook told operators to use a `force` input and `push` trigger Kata's workflow lacks, `managedCallbackReady` required the confirmed-origin marker and so showed a Repair prompt for a running fallback tunnel, `docs/user/devices.md` described iPhone Duo controls Kata disables, and the Linux updater message pointed to a `.deb` Kata does not ship.
