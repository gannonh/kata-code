...
Kata Code Connect is disabled in a fresh clone. To enable it for source builds against the production
...
When any client-facing public value is absent, cloud UI is omitted. The `katacode connect` command group is
...
deployment defaults.
`vp run --filter kata-code-relay deploy` invokes Alchemy from the relay directory, so Alchemy loads
...
3. Add **both** allowed redirect URIs:
   ...
   - `https://app.kata.sh/connect/callback` for the hosted out-of-band flow. This is
     `connectCallbackUrl(DEFAULT_HOSTED_APP_URL)` from `packages/shared/src/connectAuth.ts`, so a
     custom `KATACODE_HOSTED_APP_URL` means `$KATACODE_HOSTED_APP_URL/connect/callback` instead.
...
running Kata server. The next `katacode serve` or `katacode start` reconciles the relay link and launches the
...
logout leaves it running; manage it with `katacode service status`, `install`, `update`, and `uninstall`.
...
Local desktop development uses `katacode-dev://app`, while packaged builds use `katacode://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `katacode-dev://app`; the production Clerk instance should only need
`katacode://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
curl -X PATCH https://api.clerk.com/v1/instance \
  ...
  -d '{"allowed_origins":["katacode://app"]}'
```
...
The production macOS bundle ID is `com.katacode.app`. To enable native passkeys:

1. Create an explicit macOS App ID for `com.katacode.app` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.com.katacode.app`.
...
KATACODE_MACOS_PROVISIONING_PROFILE=/absolute/path/to/t3code.provisionprofile
...
installed app executable with `VITE_DEV_SERVER_URL` and `KATACODE_PORT` set. Rebuild the signed app
...
Signed-in users manage Kata Code Connect under **Connections**. The settings sidebar also has dedicated
controls, rendered by `SettingsSidebarNav.tsx`: `T3ConnectSidebarSignIn` in the footer shows a
**Sign in to Kata Code Connect** button while signed out, and `T3ConnectSidebarAvatar` shows a Clerk
