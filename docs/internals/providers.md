...
Kata Code-managed OpenCode chat uses one server per thread. Its MCP registrations are
directory-scoped, while Kata Code's MCP connection is thread-scoped. Sharing a chat server between
threads in one directory would
...
Kata Code auth session. The client carries the return URL back to the environment because the provider's
