...
signing only when platform credentials are present. macOS passkey builds additionally require the
`APPLE_TEAM_ID` repository variable and the `MACOS_PROVISIONING_PROFILE` secret; Windows uses Azure
Trusted Signing. Without the core signing credentials, it still releases unsigned artifacts.
