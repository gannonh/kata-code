...
[Kata Code Connect](./t3-connect.md). A relay token is never an environment login.
...
Web development environments can accept one `KATACODE_DEV_AUTH_TOKEN` across
...
the [development runbook](../operations/development.md#reuse-a-development-credential)
for setup.
