...
## 2026-08-07 (Task-wide permissions choice)

Implemented [#80](https://github.com/gannonh/kata-code/issues/80): one task-wide agent permission (Supervised / Auto-accept edits / Full access), chosen at creation and changeable from the Task panel, governing every stage session. `TaskWorkspacePreferences` gained `runtimeMode` (default Full access), `task.create` accepts it, and the new `task.permissions.set` command mirrors `task.worktree.policy.set` with the revision guard. The bootstrap saga now starts every stage thread in `preferences.runtimeMode`, and Implement's build context resolves the preference instead of hardcoding `auto-accept-edits`. `runtimeModeConfig` moved out of `ChatComposer.tsx` into `apps/web/src/runtimeModePresentation.ts` so the composer, Create Task form, and Task panel share one vocabulary. The panel also dispatches `thread.runtime-mode.set` plus a composer-draft alignment for the open stage conversation, with rejections reported beside the control. Browser tests cover creation, the checkout warning, the panel change, and error recovery; server tests prove bootstrap honoring and the next-stage handoff; the Guided `@task-workspaces` E2E asserts the new surfaces.
...
Updated the Settings connections UX to separate saved environment definitions from connectable runtime state. The former **Sandbox environments** section is now **Environments** and owns the Add Environment flow for Remote link, SSH, Docker container, and Cloud Provider definitions. The former **Remote environments** section is now **Available Runtimes** and keeps only Connect/Disconnect runtime actions; local Delete/Edit controls for saved Remote link, SSH, and sandbox runtime records moved into **Environments**. Updated browser settings coverage and sandbox creation helper test naming. Validation: `vp test apps/web/src/components/settings/SandboxDeploymentSettings.logic.test.ts`, `vp run --filter @kata-sh/code-web test:browser -- SettingsPanels.browser.tsx`, `vp run typecheck`, `vp check`, `vp run check:okf`.

## 2026-07-06 (Environments / Deployments Phase 3a — OKF finalize update)

Recorded the finalized `katacode/docker-sandbox-phase-3a` state across the bundle after the simplify (`6fd863b0b`) and strict-quality-review (`aeabe02d6`) passes. Updated the [Phase 3a deep-dive spec](/specs/2026-07-04-kata-environments-deployments-phase-3a-design.md): rewrote locked decision #4, verified-constraints table, acceptance criteria, deferred-work, and build handoff to reflect the credential model pivot from bind-mounts to copy+sanitize (`credentialSeed.ts` + `ustarWriter.ts`); appended a [Credential model deviation](/specs/2026-07-04-kata-environments-deployments-phase-3a-design.md#credential-model-deviation) section and a [Finalize outcome](/specs/2026-07-04-kata-environments-deployments-phase-3a-design.md#finalize-outcome) recording the simplify (duplicated ustar writer removal, unused constants/imports cleanup, redundant relay branch drop) and strict-quality-review (superseded `credentialBindMounts` module removal, `isSandboxEnvironment` `.sandbox` marker fix, docstring correction) passes. Updated the build completion report with the full commit list including relay unlink hardening, UI improvements, and Phase 3b retarget. Updated [ADR 0006](/adrs/0006-sandbox-provider-auth-and-railway-first-cloud-driver.md) consequences to note the local-Docker credential model evolved from bind-mounts to copy+sanitize. Updated the [deferred-work registry](/specs/deferred-work.md): Railway volume-retained resume entry closed (superseded by ADR 0007's Vercel snapshot model), Railway Sandbox VM and GHCR publish entries retargeted to ADR 0007, interactive sign-in entry updated for the copy+sanitize model and AC-3b.11. Updated the [specs roadmap](/specs/index.md) Environments/Deployments row to Phase 3a implemented + Phase 3b approved with link to the finalize outcome. **Intentionally not created:** a dedicated sandbox architecture note in `docs/architecture/` (deferred to a future dedicated OKF pass per the Phase 1/2 precedent). Logged in [specs log](/specs/log.md) and [ADR log](/adrs/log.md). Validation: `vp run check:okf` pass, `validate_okf.py` pass.

## 2026-07-05 (Environments / Deployments Phase 3b — Vercel Sandbox decision update)

Added [ADR 0007 — Vercel Sandbox as the first cloud sandbox driver](/adrs/0007-vercel-sandbox-first-cloud-sandbox-driver.md), superseding [ADR 0006](/adrs/0006-sandbox-provider-auth-and-railway-first-cloud-driver.md) for provider choice while keeping its provider-auth model. Rewrote [Phase 3b](/specs/2026-07-04-kata-environments-deployments-phase-3b-design.md) for Vercel Sandbox and updated the [Environments/Deployments roadmap](/specs/2026-06-27-kata-environments-deployments-design.md), [specs index](/specs/index.md), [ADR index](/adrs/index.md), and logs.

## 2026-07-03 (Environments / Deployments Phase 2 — OKF finalize update)

Recorded the finalized `feat/deployments-phase-2` state across the bundle after the simplify (`37e7f5404`) and strict-quality-review (`126379cd0`) passes. Appended a [Finalize outcome](/specs/2026-06-27-kata-environments-deployments-phase-2-design.md#finalize-outcome-2026-07-03) to the [Phase 2 deep-dive spec](/specs/2026-06-27-kata-environments-deployments-phase-2-design.md): behavior-preserving refactor commits (dead-param/branch trims, `disposeAfterFailure` reuse on setup-failure, double-cast removal, gitignore matcher comment correction, unused `_secretValues` param drop), the "Deployment targets → Sandbox environments" UI rename, and the two Verify-UAT deferrals ([#23](https://github.com/gannonh/kata-code/issues/23), [#24](https://github.com/gannonh/kata-code/issues/24)) already captured in the [deferred-work registry](/specs/deferred-work.md). Updated the spec's build completion report final head SHA to `126379cd0` and added the three post-build commits to its commits table. Updated the [specs roadmap](/specs/index.md) Environments/Deployments row to Phase 2 Implemented with links to the Phase 2 deep-dive and finalize outcome. **Intentionally not created:** a dedicated sandbox architecture note in `docs/architecture/` (the sandbox SPI / deployment-target layer is not yet represented in the [architecture index](/architecture/index.md) or [overview](/architecture/overview.md)) — deferred to a future dedicated OKF pass to avoid expanding scope; the Phase 1/Phase 2 deep-dive specs remain the authoritative technical record. Logged in [specs log](/specs/log.md). Validation: `vp run check:okf` pass, `validate_okf.py` pass.
