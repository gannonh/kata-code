...
stdout/stderr at `~/.katacode/ssh-launch/<state>/server.log`.
...
- SSH-managed launch persistence: `~/.katacode/ssh-launch/<state>/server.log`
...
`<home>/userdata/logs/server.trace.ndjson` (so `~/.katacode/userdata/...` by default, or
`/custom/path/userdata/...` with `--home-dir /custom/path`), a linked worktree dev run uses
`<worktree>/.katacode/userdata/logs/server.trace.ndjson`, and an implicit dev run outside a linked
worktree uses `~/.katacode/dev/logs/server.trace.ndjson`.
...
`katacode trace summary` reads the trace file and its rotated backups directly, so it works while the
...
```bash
katacode trace summary --since 30m --limit 40
```

It reads `KATACODE_TRACE_FILE` if set, else `<home>/userdata/logs/server.trace.ndjson` for
`--base-dir` or `KATACODE_HOME`, plus the `KATACODE_TRACE_MAX_FILES` rotated backups. For a dev run or
a copied file, set `KATACODE_TRACE_FILE`. `--since 30m` keeps spans that ended in the last 30
...
`KATACODE_TRACE_MIN_LEVEL` is `Warn`. The warning shows in Settings > Diagnostics unless OTLP logs are
...
export KATACODE_OTLP_LOGS_URL=http://localhost:4318/v1/logs
...
KATACODE_OTLP_LOGS_URL=http://localhost:4318/v1/logs \
...
5. Look for spans from the `katacode-server` or `katacode-desktop` service, then narrow by span name or
   attributes.

Good first searches:

- service name `katacode-server` or `katacode-desktop`, plus a resource attribute such as
...
`apps/desktop/src/app/DesktopObservability.ts`. It reads the same `KATACODE_OTLP_*` names and the same
...
`katacode-desktop`, so a collector shows it alongside the backend rather than mixed into it. It
...
- `KATACODE_OTLP_LOGS_URL`: OTLP log endpoint
- `KATACODE_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `KATACODE_OTLP_HEADERS`: extra headers for all three exporters, same format as
  `OTEL_EXPORTER_OTLP_HEADERS`: comma-separated `key=value` pairs with percent-encoded values.
- `KATACODE_OTLP_PROTOCOL`: `http/json` (default) or `http/protobuf`
...
non-blank `KATACODE_OTLP_*_URL` wins over either, and a per-signal endpoint wins over the generic one
...
signal. `KATACODE_OTLP_HEADERS` and `KATACODE_OTLP_PROTOCOL` never apply to it. An endpoint that is not
...
Service names are fixed: `katacode-server` for the backend and `katacode-desktop` for the desktop main
process, both in `service.namespace` `katacode`. `OTEL_SERVICE_NAME` and a `service.name` or
...
`KATACODE_OTEL_SDK_DISABLED` and `OTEL_SDK_DISABLED` turn off every OTLP export in both the server and
...
`KATACODE_OTEL_SDK_DISABLED` wins when set, so `KATACODE_OTEL_SDK_DISABLED=false` re-enables export on a
...
snapshot to its logs dir and logs the path. This works for desktop, `npx @kata-sh/code-cli`, and service installs
...
```bash
pid="$(jq .pid "${KATACODE_HOME:-$HOME/.katacode}/userdata/server-runtime.json")"
ps -p "$pid" -o command=
```

If `ps` shows the Kata Code server, send the signal:
