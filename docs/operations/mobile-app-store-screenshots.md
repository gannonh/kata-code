...
the software keyboard, and supplies deterministic Kata Code Connect discovery rows to the real
...
`pnpm --filter @kata-sh/code-mobile showcase` starts Expo on its normal port, so it is not compatible with
...
The Pending rows use the production offline outbox and point at the real Kata Code and React fixture
