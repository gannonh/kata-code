...
Labeling a PR `preview:mac` publishes a signed, notarized Apple Silicon DMG with Kata Code Connect enabled
...
- `.github/workflows/desktop-macos-preview-publish.yml` runs on `workflow_run` from `main`. It
  ...
  the PR. Only the version and the public Kata Code Connect identifiers from 1Password are read from the
...
stage, test Cloudflare account, disposable host, and disposable Kata Code home. Keep production cleanup at
`off` or `dry-run` until it passes. Do not stop a daily-use Kata Code server.
...
2. Stop that host and restart the same Kata Code home on a different local port. Confirm the public
