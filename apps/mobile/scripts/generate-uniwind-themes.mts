...
import { BUILT_IN_THEME_IDS, type BuiltInThemeId } from "@kata-sh/code-shared/themePalettes";
...
if (import.meta.main) {
  ...
  for (const [filename, contents] of getGeneratedUniwindThemeOutputs()) {
    if (checkOnly) {
      ...
      if (current !== contents) {
        console.error(
          `${NodePath.relative(process.cwd(), filename)} is stale. Run vp run --filter @kata-sh/code-mobile generate.`,
        );
        process.exitCode = 1;
      }
      continue;
