...
describe("shouldHandleAppLink", () => {
  it.each(["katacode://", "katacode:///", "katacode-dev://", "katacode-preview://"])(
  ...
  it.each([
    "katacode://threads/env-1/thread-1",
    "katacode://pair?pairingUrl=x",
    "katacode-dev://settings/usage?tab=limits",
  ])("handles path-bearing URL %s", (url) => {
    expect(shouldHandleAppLink(url)).toBe(true);
  });

  it.each(["katacode://expo-development-client/?url=x", "katacode://expo-sharing/anything"])(
