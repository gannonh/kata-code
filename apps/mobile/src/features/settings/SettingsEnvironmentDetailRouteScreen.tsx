...
function EnvironmentDetail({ environmentId }: { readonly environmentId: EnvironmentId }) {
  ...
  function requestServerUpdate() {
    ...
    Alert.alert(
      ...
      `Install ${APP_BASE_NAME} ${targetVersion}. ${capabilities.serverSelfUpdate === "desktop-managed" ? "The desktop app will close and relaunch." : "The server will restart and reconnect."} Running threads may be interrupted.`,
      [
      ...
      >
        {!environment ? (
          ...
          <>
            ...
            {config ? (
              <>
                <SettingsSection title={APP_BASE_NAME}>
                    ...
                    {!supportsEnvironmentUpdate(config.environment.capabilities) ? (
                      <Text className="text-sm text-foreground-muted">
                        {capabilities?.serverSelfUpdate === "desktop-managed"
                          ...
                          : `Update and restart ${APP_BASE_NAME} on this machine.`}
                      </Text>
                    ) : null}
                  </View>
                  <SettingsActionRow
