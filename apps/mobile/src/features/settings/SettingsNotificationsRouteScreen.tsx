...
import { APP_BASE_NAME, CLOUD_PRODUCT_NAME } from "@kata-sh/code-shared/branding";
      ...
      <SettingsScreen title="Notifications">
        ...
        >
          <Text className="text-base text-foreground-muted">
            Notifications require {CLOUD_PRODUCT_NAME} in this app build.
          </Text>
        </ScrollView>
      </SettingsScreen>
    );
    ...
    async (enabled: boolean): Promise<boolean> => {
      ...
      if (result._tag === "Failure") {
        if (!isAtomCommandInterrupted(result)) {
          reportAtomCommandResult(result, { label: "live activity preference save" });
        }
        return false;
      }
      return true;
    ...
    if (result.value.type === "granted") {
      ...
      if (getAgentAwarenessRegistrationStatus() === "registered") {
        ...
        Alert.alert(
          ...
          `Notification access was granted, but this device could not be registered with ${CLOUD_PRODUCT_NAME}. Notifications will start once registration succeeds.`,
        );
      }
      return;
    }
  ...
  const linkEnvironments = useCallback(async () => {
    ...
    if (getAgentAwarenessRegistrationStatus() === "registered") {
      ...
      Alert.alert(
        ...
        `This device could not be registered with ${CLOUD_PRODUCT_NAME}, so activity updates won't appear yet. They'll start once registration succeeds.`,
      );
    }
  }, [
    connections,
