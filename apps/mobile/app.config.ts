...
const isIosPersonalTeamBuild = repoEnv.KATACODE_IOS_PERSONAL_TEAM === "1";
const runtimeVersionPolicy =
  process.env.MOBILE_VERSION_POLICY ??
  (APP_VARIANT === "development" ? "appVersion" : "fingerprint");

const personalTeamBundleIdentifier = repoEnv.KATACODE_IOS_PERSONAL_TEAM_BUNDLE_ID?.trim();
...
if (
  ...
  throw new Error(
    "KATACODE_IOS_PERSONAL_TEAM_BUNDLE_ID must be a reverse-DNS identifier such as com.example.katacode when KATACODE_IOS_PERSONAL_TEAM=1.",
  );
}
    ...
    widgets: [
      {
        ...
        description: "Subscription quotas from your connected Kata Code environments.",
        configuration: {
...
const config: ExpoConfig = {
  ...
  updates: {
    enabled: repoEnv.KATACODE_MOBILE_UPDATES_ENABLED !== "0",
  ...
  ios: {
    ...
    infoPlist: {
      ...
      NSPhotoLibraryAddUsageDescription: "Allow Kata Code to save images to your photo library.",
  ...
  android: {
    ...
    ...(repoEnv.KATACODE_ANDROID_GOOGLE_SERVICES_FILE
      ? { googleServicesFile: repoEnv.KATACODE_ANDROID_GOOGLE_SERVICES_FILE }
      : {}),
    adaptiveIcon: {
  ...
  plugins: [
    ...
    [
      "expo-audio",
      {
        microphonePermission: "Allow Kata Code to use your microphone for voice input.",
