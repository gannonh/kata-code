...
} from "@kata-sh/code-client-runtime/platform";
import {
...
function clientMetadata() {
  ...
  return {
    label: desktop ? "Kata Code Desktop" : "Kata Code Web",
    ...
    const cloudSession = CloudSession.of({
      clerkToken: Effect.gen(function* () {
        ...
        if (session === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "Sign in to Kata Code Connect to connect this environment.",
          });
        }
        ...
        if (token === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "The Kata Code Connect session is unavailable.",
          });
        }
        return token;
      }),
