      ...
      toastManager.add({
        ...
        description:
          selfUpdate === "desktop-managed"
            ...
            : `Reconnected on @kata-sh/code-cli@${result.value.targetVersion}.`,
      });
    } catch (error) {
      toastManager.add({
        type: "error",
      ...
      if (desktopTargets.length > 0) {
        const confirmed =
          (await requestConfirmDialog(
            `Update the Kata Code desktop apps on ${desktopTargets.map((target) => target.serverLabel).join(", ")}? They will close and relaunch on those machines.`,
          )) ?? true;
        if (!confirmed) return;
      }
      await Promise.all(
...
export function ServerUpdateAction({
  ...
  const handleUpdate = async () => {
    ...
    if (isDesktopAppUpdate) {
      ...
      const confirmed =
        (await requestConfirmDialog(
          `Update the Kata Code desktop app that runs the ${serverLabel}? It will close and relaunch on that machine.`,
        )) ?? true;
      if (!confirmed) {
        return;
      }
