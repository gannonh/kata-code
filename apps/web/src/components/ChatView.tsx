...
import type { UsageLimitSourceSnapshots } from "@kata-sh/code-contracts";
import {
  collectProviderUsageLimits,
  hasProviderUsageLimits,
  isUsageLimitsCommand,
} from "@kata-sh/code-shared/usageLimits";
import { feedbackBannerItem } from "./chat/ComposerFeedback";
import { usageLimitsBannerItem } from "./chat/ComposerUsageLimits";
import { derivePendingRequests } from "@kata-sh/code-client-runtime/pending-requests";
import {
...
import { isPasteAsTextShortcut } from "@kata-sh/code-client-runtime/text-paste";
import { type CodexArtifactTemplate } from "@kata-sh/code-client-runtime/codex-artifact-templates";
import { effectiveSnoozed, threadWokeAt } from "@kata-sh/code-client-runtime/state/thread-settled";
import {
...
import { resolveThreadReferenceCopyTarget } from "@kata-sh/code-shared/threadReference";
import {
  getTerminalLabel,
  nextTerminalId,
...
import { serializeLegacyContextMessage } from "@kata-sh/code-shared/composerContextLegacySend";
import {
  buildMessageContext,
...
import { resolveProviderSkillsForCwd } from "@kata-sh/code-client-runtime/providerSkills";
import { vcsEnvironment } from "../state/vcs";
import { sourceControlEnvironment } from "../state/sourceControl";
import { useProjectClone } from "../state/projectClones";
import { projectCloneDisplayName, projectCloneProgressSummary } from "@kata-sh/code-contracts";
...
import { clampFileAttachmentUploadBytes } from "@kata-sh/code-client-runtime/state/attachments";
  ...
  const onRespondToUserInput = useCallback(
    async (requestId: ApprovalRequestId, answers: Record<string, unknown>) => {
      ...
      const attachmentsByQuestionId = new Map<
        string,
        import("@kata-sh/code-contracts").UserInputAttachments[string]
      >();
      for (const question of activePendingUserInput.questions) {
        ...
        attachmentsByQuestionId.set(
          question.id,
          uploaded as import("@kata-sh/code-contracts").UserInputAttachments[string],
        );
      }
  ...
  const rightPanelContent = activeThreadRef ? (
    renderedRightPanelSurface?.kind === "preview" ? (
      ...
      <PullRequestsUnavailableState
        ...
        error="Update this environment's Kata Code server to browse pull requests."
      />
