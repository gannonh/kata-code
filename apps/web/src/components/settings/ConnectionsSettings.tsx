...
import { connectionStatusText } from "@kata-sh/code-client-runtime/connection";
import {
...
function CloudLinkSwitch({
  ...
  disabledReason,
  onCheckedChange,
  ariaLabel = "Enable Kata Code Connect",
}: {
...
function ConfiguredCloudLinkRow({ canManageRelay }: { readonly canManageRelay: boolean }) {
  ...
  const disabledReason = !isSignedIn
    ? "Sign in to Kata Code Connect to manage this environment."
    : !canManageRelay
  ...
  const updateManagedTunnel = async (enabled: boolean) => {
    ...
    if (ok) {
      ...
      toastManager.add({
        type: "success",
        title: enabled
          ? "Kata Code Connect linked"
          : publishAgentActivity
            ? "Kata Code Connect tunnel disabled"
            : "Kata Code Connect unlinked",
        description: enabled
  ...
  return (
    <>
      ...
      <SettingsRow
        ...
        description="Send activity from this environment to your mobile clients for push notifications and Live Activities. Works without a Kata Code Connect tunnel."
        control={
          <CloudLinkSwitch
  ...
  return (
    <Empty className="min-h-52">
      ...
      <EmptyHeader>
        ...
        <EmptyDescription>
          {cloudEnabled
            ? "Click “Add environment” to pair another environment, or connect one from Kata Code Connect."
    ...
    return (
      <>
        ...
        {desktopWslState.enabled ? (
          <SettingsRow
            title="WSL only"
            description="Run only the WSL backend. Kata Code restarts when this changes."
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingDesktopServerExposureMode === "network-accessible"
                    ? "Let your other devices connect to Kata Code over the network. Pair devices to give them access. Kata Code will restart."
                    : "Devices connected over your local network will disconnect. Existing tunnels, such as Kata Code Connect or Tailscale HTTPS, keep working. Kata Code will restart."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
            ...
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingWslChange?.kind === "disable"
                    ? pendingWslChange.wasWslOnly
                      ? "Kata Code will restart on the Windows backend. Threads and projects opened against WSL stay safe inside the distro and become available again when you re-enable WSL."
                      : "The WSL backend will stop. Threads and projects opened against WSL stay safe inside the distro, but they'll be unavailable in Kata Code until you re-enable WSL."
                    : pendingWslChange?.kind === "distro"
                      ? "Kata Code will restart the WSL backend on the new distro. Sessions still running on the current distro will be interrupted."
                      : pendingWslChange?.kind === "enable"
                        ? "Run the WSL backend alongside the Windows one, or stop the Windows backend and use only WSL? You can change this later from Settings."
                        : pendingWslChange?.nextValue
                          ? "Kata Code will restart and start only the WSL backend. Your Windows-side projects won't be accessible until you turn this off again."
                          : "Kata Code will restart and bring the Windows backend back up alongside WSL."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
