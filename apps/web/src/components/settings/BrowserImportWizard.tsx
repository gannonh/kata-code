...
import type { BrowserImportSource } from "@kata-sh/code-contracts";
import { BROWSER_IMPORT_FAILURE_COPY } from "@kata-sh/code-contracts";
...
function FullDiskAccessStep({
  ...
  return (
    <>
      <DialogHeader>
        <DialogTitle>Let Kata Code read {source.name}&rsquo;s cookies</DialogTitle>
        <DialogDescription>
          To import cookies from {source.name}, Kata Code needs Full Disk Access. Turn it on in
      ...
      <DialogPanel>
        ...
        {!permission.isReady(["fullDiskAccess"]) ? (
          <p className="mt-3 text-xs text-muted-foreground">
            {stillRequired
              ? "Access is still required. Quit and reopen Kata Code if you just allowed it, then retry the import."
              : "If access doesn't update after you allow it, quit and reopen Kata Code, then retry the import."}
          </p>
        ) : null}
      </DialogPanel>
      <DialogFooter>
