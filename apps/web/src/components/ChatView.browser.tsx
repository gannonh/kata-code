...
function createMockEnvironmentApi(input: {
  ...
  return {
    ...
    taskWorkspaces: {} as EnvironmentApi["taskWorkspaces"],
    orchestration: {
  ...
  it("renders locked single-environment mobile run context as a static workspace label", async () => {
    ...
    try {
      ...
      const label = await waitForElement(
        () => document.querySelector<HTMLElement>('[data-mobile-run-context-label="true"]'),
        "Unable to find static mobile workspace label.",
      );

      expect(label.textContent?.trim()).toBe("Project");
