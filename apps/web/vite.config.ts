...
import { DEV_PROXIED_PATH_PREFIXES } from "@kata-sh/code-shared/devProxy";
...
// as hot patches. Opt-in while experimental: KATACODE_BUNDLED_DEV=1 pnpm dev:web
// The dev runner defaults this on for --share runs (remote browsers pay a
// round trip per import level in unbundled dev); KATACODE_BUNDLED_DEV=0 opts out.
const bundledDevEnv = process.env.KATACODE_BUNDLED_DEV?.trim().toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // KATACODE_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
...
const devProxyTarget = resolveDevProxyTarget(process.env.KATACODE_PORT, configuredWsUrl);
...
const configuredAllowedHosts = (process.env.KATACODE_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
