...
const packExecutable = process.env.KATACODE_PACK_EXE === "1";
...
const packExecutableTarget = process.env.KATACODE_PACK_EXE_TARGET?.trim();
if (packExecutableTarget && !Object.hasOwn(SEA_TARGETS, packExecutableTarget)) {
  throw new Error(
    `KATACODE_PACK_EXE_TARGET must be one of ${Object.keys(SEA_TARGETS).join(", ")}, got "${packExecutableTarget}".`,
  );
}
...
export default mergeConfig(
  ...
  defineConfig({
    ...
    pack: {
      ...
      define: {
        __KATACODE_BUILD_CHANNEL__: JSON.stringify(cliBuildChannel),
