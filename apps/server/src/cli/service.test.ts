...
const status = {
  ...
  logPath: "/home/me/.katacode/userdata/logs/boot-service.log",
} as const;
...
it("reports the installed service version and host paths", () => {
  assert.equal(
    ...
    [
      "Kata Code service",
      "  Status: installed · @kata-sh/code-cli@0.0.29",
      "  Unit: /home/me/.config/systemd/user/t3code.service",
      "  Logs: /home/me/.katacode/userdata/logs/boot-service.log",
    ].join("\n"),
  );
});
...
it("gives a direct repair command for a stale service", () => {
  assert.include(
    ...
    "Next: Run `npx @kata-sh/code-cli@0.0.29 service update`.",
  );
});
...
it("explains an incomplete nightly installation and keeps repair on its installed version", () => {
  ...
  expect(output).toContain("npx @kata-sh/code-cli@0.0.32-nightly.1 service update");
  expect(output).not.toContain("@kata-sh/code-cli@latest");
});
...
it("suggests the newer CLI version when the installed service needs an update", () => {
  ...
  expect(output).toContain("npx @kata-sh/code-cli@0.0.29 service update");
  expect(output).not.toContain("npx @kata-sh/code-cli@0.0.28 service update");
});
...
it("reports a newer installed service and gives an exact-version repair command", () => {
  ...
  assert.include(
    output,
    "@kata-sh/code-cli@0.0.32-nightly.1 (newer than this @kata-sh/code-cli@0.0.31 CLI)",
  );
  assert.include(output, "npx @kata-sh/code-cli@0.0.32-nightly.1 service update");
  assert.notInclude(output, "npx @kata-sh/code-cli@latest service update");
});
