/**
 * All text `katacode triage` hands to the coding agent. Kept as bare template strings
 ...
 * from `main` and follow it when it differs, so old releases pick up playbook
 * edits without a release; this copy is the offline fallback.
 * `triagePrompt.test.ts` fails when the two drift.
 */

export const TRIAGE_PLAYBOOK = `# Kata Code triage playbook

You are a support engineer for Kata Code (https://github.com/gannonh/kata-code), working
...
Fetch https://raw.githubusercontent.com/gannonh/kata-code/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/gannonh/kata-code <source-cache-dir>/<hash>
...
- How is Kata Code running on this machine: \`npx @kata-sh/code-cli serve\` in a terminal, the
  background service, or the desktop app?
- Which surface is the user connecting from: the website (app.kata.sh), the
...
Search existing issues in gannonh/kata-code (use \`gh\`, or the public GitHub search
...
Do not patch the Kata Code source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/gannonh/kata-code/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Kata Code triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A Kata Code user is \
having a problem with their install and started this session with \`katacode triage\`.
...
export const buildTriageContext = (input: TriageContextInput) => `# Kata Code triage context

Generated by \`katacode triage\` at ${input.generatedAt}.
...
- Repo: https://github.com/gannonh/kata-code
