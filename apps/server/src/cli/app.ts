...
import { resolveDesktopAppControlAddress } from "@kata-sh/code-shared/desktopAppControl";
import {
  HostProcessPlatform,
  HostProcessUserId,
...
export class DesktopAppSshUnsupportedError extends Schema.TaggedError<DesktopAppSshUnsupportedError>()(
  ...
  override get message(): string {
    return "`katacode app` only controls a desktop app on the same machine. It cannot run over SSH.";
  }
}
...
export class DesktopAppPlatformUnsupportedError extends Schema.TaggedError<DesktopAppPlatformUnsupportedError>()(
  ...
  override get message(): string {
    return `\`katacode app\` is not supported on ${this.platform}.`;
  }
}
...
export class DesktopAppUnreachableError extends Schema.TaggedError<DesktopAppUnreachableError>()(
  ...
  override get message(): string {
    return "Could not reach the Kata Code desktop app. Start or update the desktop app on this machine, then run `katacode app` again. A running Kata Code server is not enough.";
  }
}
...
export class DesktopAppRequestFailedError extends Schema.TaggedError<DesktopAppRequestFailedError>()(
  ...
  override get message(): string {
    return `Kata Code could not open ${this.workspaceRoot} (${this.code}).`;
  }
}
...
const runAppCommand = Effect.fn("cli.app")(function* (flags: {
  ...
  yield* Console.log(`Opened ${workspaceRoot} in Kata Code.`);
});
...
export const appCommand = Command.make("app", {
  ...
  Command.withDescription("Open a project in the running Kata Code desktop app."),
