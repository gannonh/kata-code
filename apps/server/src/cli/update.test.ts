...
it.layer(NodeServices.layer)("katacode update launcher", (it) => {
  it.effect("repoints a symlink that lives in a runtime versions tree", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "katacode-update-" });
      const oldExe = path.join(root, "runtime/versions/1.0.0/katacode");
      const newExe = path.join(root, "runtime/versions/2.0.0/katacode");
      const launcher = path.join(root, "bin/katacode");
  ...
  it.effect("leaves a plain copy or a foreign symlink alone", () =>
    Effect.gen(function* () {
      ...
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "katacode-update-" });
      const newExe = path.join(root, "runtime/versions/2.0.0/katacode");
      const copy = path.join(root, "copy/katacode");
      const foreign = path.join(root, "foreign/katacode");
      const elsewhere = path.join(root, "elsewhere/katacode");
      // Another install's versions tree: same shape, different home.
      const otherHome = path.join(root, "other/runtime/versions/1.0.0/katacode");
      const otherLauncher = path.join(root, "other/bin/katacode");
  ...
  it.effect("finds the launcher a bare command name resolved to on PATH", () =>
    Effect.gen(function* () {
      ...
      const bare = yield* resolveLauncherPath.pipe(
        Effect.provideService(HostProcessInvokedAs, "katacode"),
      ...
      const relative = yield* resolveLauncherPath.pipe(
        Effect.provideService(HostProcessInvokedAs, "./bin/katacode"),
