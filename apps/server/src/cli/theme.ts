...
/**
 * `katacode theme` - inspect and set the environment's theme. Connected web and
 ...
 * Settings afterwards sticks until the next `katacode theme set`.
 *
...
import { BUILT_IN_THEME_IDS, UNPUBLISHABLE_THEME_IDS } from "@kata-sh/code-shared/themePalettes";
...
export class ThemeFileInvalidError extends Schema.TaggedError<ThemeFileInvalidError>()(
  ...
  override get message(): string {
    return `${this.filePath} is not a valid theme file. Use a theme exported from Kata Code, or a seeded file with name, appearance, canvas, and accent.`;
  }
}
...
export class ThemeTargetMissingError extends Schema.TaggedError<ThemeTargetMissingError>()(
  ...
  override get message(): string {
    return "Provide a theme id or file, or run `katacode theme clear` to remove the theme.";
  }
}

const envT3Home = Config.string("KATACODE_HOME").pipe(Config.option);

const resolveThemePaths = Effect.fn(function* (explicitBaseDir: Option.Option<string>) {
  // Same precedence as the rest of the CLI: --base-dir, then KATACODE_HOME,
  // then the default home. A provisioning script exporting KATACODE_HOME must
...
const themeSetCommand = Command.make("set", {
  ...
  Command.withHandler((flags) =>
    Effect.gen(function* () {
      ...
      // make `katacode theme set ocean` publish ./ocean whenever the cwd happens to
