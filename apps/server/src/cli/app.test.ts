...
import type { DesktopAppActivationRequest } from "@kata-sh/code-contracts";
...
describe("katacode app", () => {
    ...
    withTempDirectory("t3-app-ssh-test-", (root) =>
      Effect.gen(function* () {
        ...
        expect(error).toMatchObject({
          ...
          message:
            "`katacode app` only controls a desktop app on the same machine. It cannot run over SSH.",
        });
    ...
    withTempDirectory("t3-app-platform-test-", (root) =>
      Effect.gen(function* () {
        ...
        expect(error).toMatchObject({
          ...
          platform: "freebsd",
          message: "`katacode app` is not supported on freebsd.",
        });
        expect(yield* pathExists(baseDir)).toBe(false);
      }),
    ),
    ...
    withTempDirectory("t3-app-missing-test-", (root) =>
      Effect.gen(function* () {
        ...
        expect(error).toMatchObject({
          ...
          message: expect.stringContaining("Could not reach the Kata Code desktop app."),
          cause: { code: "ENOENT" },
        });
        expect(yield* pathExists(baseDir)).toBe(false);
      }),
  ...
  it.effect("uses KATACODE_HOME or --base-dir and sends the default or explicit path", () =>
    withTempDirectory("t3-app-command-test-", (root) =>
      Effect.gen(function* () {
        ...
        yield* runCli(["app"], { KATACODE_HOME: baseDir });
  ...
  it.effect("prefers the installed desktop app when a dev desktop is also running", () =>
    withTempDirectory("t3-app-preferred-test-", (root) =>
      Effect.gen(function* () {
        ...
        const baseDir = NodePath.join(root, ".katacode");
        const desktop = yield* fakeDesktop({ baseDir });
  ...
  it.effect("finds the dev desktop when the default desktop socket is absent", () =>
    withTempDirectory("t3-app-dev-test-", (root) =>
      Effect.gen(function* () {
        ...
        yield* runCli(["app"]);
        yield* runCli(["app"], { KATACODE_HOME: "   " });
  ...
  it.effect("never searches a dev state directory for an explicit T3 home", () =>
    withTempDirectory("t3-app-explicit-test-", (root) =>
      Effect.gen(function* () {
        ...
        const envError = yield* runCli(["app"], { KATACODE_HOME: baseDir }).pipe(Effect.flip);
