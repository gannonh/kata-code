...
it.layer(NodeServices.layer)("cli config resolution", (it) => {
    ...
    Effect.gen(function* () {
      ...
      const configLayer = ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            KATACODE_DEV_AUTH_TOKEN: "  reusable-dev-auth-token-that-is-long-enough  ",
          },
        }),
      );
  ...
  it.effect("does not expose an invalid reusable auth token", () =>
    Effect.gen(function* () {
      ...
      const configLayer = ConfigProvider.layer(
        ConfigProvider.fromEnv({ env: { KATACODE_DEV_AUTH_TOKEN: secret } }),
      );
  ...
  it.effect(
    "lets KATACODE_OTEL_SDK_DISABLED=false override an ambient OTEL_SDK_DISABLED=true",
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        ...
        Effect.provide(
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  KATACODE_OTLP_HEADERS: "authorization=Bearer abc==, x-tenant=t3",
                  KATACODE_OTLP_TRACES_URL: "http://collector.internal:4318",
                },
              }),
            ),
            NetService.layer,
    ...
    Effect.gen(function* () {
      ...
      const resolved = yield* resolveServerConfig(
        ...
        Effect.provide(
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({ env: { KATACODE_OTLP_PROTOCOL: "http/protobuf" } }),
            ),
            NetService.layer,
          ),
        ),
      ...
      const resolved = yield* resolveServerConfig(
        ...
        Effect.provide(
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: { KATACODE_OTLP_LOGS_URL: "http://collector.internal:4318/v1/logs" },
              }),
            ),
            NetService.layer,
          ),
  ...
  it.effect(
    "resolves each signal's endpoint through KATACODE_OTLP_*_URL, an OTEL endpoint, the bootstrap envelope, and persisted Settings, in that order",
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
          ...
          Effect.provide(
            Layer.mergeAll(
              ConfigProvider.layer(
                ConfigProvider.fromEnv({
                  env: {
                    KATACODE_OTLP_TRACES_URL: "http://t3:4318/v1/traces",
                    KATACODE_OTLP_HEADERS: "x-key=secret",
        ...
        // KATACODE_OTLP_TRACES_URL wins over the OTEL variable for the same
        // signal, and keeps Kata Code's own headers since Kata Code still owns it.
        expect(resolved.otlpTracesUrl).toBe("http://t3:4318/v1/traces");
        expect(resolved.otlpTracesExport.headers).toEqual({ "x-key": "secret" });
        // Metrics named no KATACODE_OTLP_METRICS_URL, so the OTEL endpoint wins
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        // KATACODE_OTLP_TRACES_URL still wins outright.
        expect(resolved.otlpTracesUrl).toBe("http://t3:4318/v1/traces");
        // The OTEL endpoint claimed metrics and logs, so neither the bootstrap
        // envelope nor Settings receives them with Kata Code's headers.
        expect(resolved.otlpMetricsUrl).toBeUndefined();
        expect(resolved.otlpLogsUrl).toBeUndefined();
      }),
  );
