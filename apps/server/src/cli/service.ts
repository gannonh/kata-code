...
export function formatServiceStatus(
  ...
  if (
    ...
    return [
      ...
      `  Logs: ${status.logPath}`,
      ...problems,
      `  Next: Run \`katacode update ${installedVersion}\` to match it, or pass \`--allow-downgrade\` to \`katacode service install\` explicitly.`,
    ].join("\n");
  }
  return [
    "Kata Code service",
    ...
    `  Logs: ${status.logPath}`,
    ...problems,
    ...(status.current ? [] : ["  Next: Run `katacode service install` to repair it."]),
  ].join("\n");
}
...
// Kept one release for muscle memory and old docs. It did what `katacode service
// install` does; the way to move to a newer release is `katacode update`.
const serviceUpdateCommand = Command.make("update", serviceReconcileFlags).pipe(
  Command.withDescription("Deprecated. Run `katacode update` to move to a newer release."),
  Command.unlisted,
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        yield* Console.log(
          "`katacode service update` is deprecated: run `katacode update` to move to a newer release, or `katacode service install` to repair the service. Repairing now.",
        );
        const result = yield* reconcileService({ allowDowngrade: flags.allowDowngrade });
        if (!result.changed) {
          yield* Console.log(
...
const serviceRestartCommand = Command.make("restart", projectLocationFlags).pipe(
  Command.withDescription(
    "Restart the background service. Picks up a version installed by `katacode update` that was not restarted at the time.",
  ),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        ...
        yield* Console.log(
          restarted
            ? `Restarted the Kata Code service${status.installedVersion === undefined ? "" : ` on @kata-sh/code-cli@${status.installedVersion}`}.`
            : "Kata Code service is not installed.",
        );
      }),
    ),
  ),
...
const serviceUninstallCommand = Command.make("uninstall", projectLocationFlags).pipe(
  Command.withDescription("Stop and remove the Kata Code background service."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        ...
        yield* Console.log(
          removed ? "Removed the Kata Code service." : "Kata Code service is not installed.",
        );
      }),
    ),
  ),
...
const serviceStatusCommand = Command.make("status", projectLocationFlags).pipe(
  Command.withDescription("Show whether the Kata Code background service is installed."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
...
export const serviceCommand = Command.make("service").pipe(
  Command.withDescription("Manage the Kata Code background service."),
