...
function formatCloudStatus(status: CloudCliStatus, options?: { readonly json?: boolean }): string {
  ...
  return [
    ...
    "",
    "This is saved setup, not a live connection check. Check the background service with `katacode service status`.",
...
const confirmRelayClientInstall = (version: string) =>
  Prompt.run(
    Prompt.confirm({
      message: `The Kata Code relay client is required for Kata Code Connect. Download and install version ${version}?`,
      initial: false,
    }),
  );
    ...
    runCloudCommand(
      ...
      Effect.gen(function* () {
        ...
        if (linked) {
          ...
          yield* Console.log(
            flags.publishOnly
              ? `✓ Authorized${connectedAs(linked.identity)}\n\nNext\n  Start Kata Code to publish agent activity (no managed tunnel).`
    ...
    runCloudCommand(
      ...
      Effect.gen(function* () {
        ...
        if (yield* CliState.readCliDesiredCloudLink) {
          yield* Console.log(
            "A Kata Code Connect link is already pending. Start Kata Code to finish provisioning it; publishing starts once it links.",
          );
          return;
        }
        yield* CliState.setCliDesiredCloudLink(true, "publish_only");
        yield* Console.log(
          "Restart Kata Code to finish authorizing this environment to publish (no managed tunnel is created).",
        );
      }),
    ),
  ),
    ...
    runCloudCommand(
      ...
      Effect.gen(function* () {
        ...
        if (background) {
          ...
          yield* Console.log(
            platform === "darwin"
              ? "\n✓ Background service ready\n\nKata Code is set to run while you are logged in to this Mac. The server establishes the Kata Code Connect link on startup."
              : "\n✓ Background service ready\n\nKata Code is set to keep running after you log out. The server establishes the Kata Code Connect link on startup.",
          );
          return;
        }
        const serveCommand = yield* resolveCliCommand("serve");
