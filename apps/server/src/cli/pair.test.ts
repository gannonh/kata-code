  ...
  Effect.acquireUseRelease(
    Effect.callback<NodeHttp.Server>((resume) => {
      const server = NodeHttp.createServer((request, response) => {
        if (request.url === "/.well-known/kata/environment") {
...
describe("katacode pair", () => {
  it.effect("mints a token and prints a QR pairing URL for a live server", () =>
    withDescriptorServer((origin) =>
      Effect.gen(function* () {
        ...
        assert.equal(credentials[0]?.label, "katacode pair");
      }),
  ...
  it.effect("directs to t3 serve or t3 connect when no server is running", () =>
    Effect.gen(function* () {
      ...
      assert.include(rendered, "No running Kata Code server found.");
      assert.include(rendered, "npx katacode serve");
      assert.include(rendered, "npx katacode connect");
    }).pipe(Effect.provide(NodeServices.layer)),
  );
