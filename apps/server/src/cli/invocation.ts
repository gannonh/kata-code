 ...
 * Detection is best-effort; callers must fail closed to a plain `katacode` command.
 */
 ...
 * `@kata-sh/code-cli@nightly`) is resolved away before our process starts, so re-derive it
...
function suggestedPackageSpec(version: string): string {
  ...
  return channel === undefined ? "@kata-sh/code-cli" : `@kata-sh/code-cli@${channel}`;
}

/**
 * Render a `katacode <subcommand>` suggestion that matches how this process was
 * launched, so copy/pasting it actually works: `npx katacode connect` suggests
 * `npx katacode serve`, a global install suggests `katacode serve`, and a nightly build
...
export function formatCliCommand(input: {
  ...
  if (runner === null) {
    return `katacode ${input.subcommand}`;
  }
