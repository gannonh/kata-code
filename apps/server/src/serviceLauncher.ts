...
const runtimePaths = (baseDir: string, version: string) => {
  ...
  // oxlint-disable-next-line kata-code/no-global-process-runtime -- Standalone launcher has no Effect runtime.
  const executableName = process.platform === "win32" ? "katacode.exe" : "katacode";
  return {
    versionDir,
...
export class Launcher {
  ...
  async #recover(): Promise<void> {
    ...
    // handoff release its tunnel. A restart deferred by `katacode update` is done
  ...
  async #startChild(version: string, role: ChildRole, update?: ServiceUpdateRecord): Promise<void> {
    ...
    if (!(await runtimeExists(this.#baseDir, version))) {
      throw new Error(`Selected @kata-sh/code-cli@${version} runtime is missing or incomplete.`);
    }
    if (this.#stopping) return;
...
export async function main(): Promise<void> {
  const baseDir = process.env.KATACODE_HOME?.trim();
  if (baseDir === undefined || baseDir === "") {
    throw new Error("KATACODE_HOME is required by the Kata Code service launcher.");
  }
