...
import type { CursorSettings, ServerProviderUsageWindow } from "@kata-sh/code-contracts";
import { HostProcessPlatform } from "@kata-sh/code-shared/hostProcess";
import { CURSOR_USAGE_WINDOWS } from "@kata-sh/code-shared/usageLimits";
    ...
    if (!token && platform === "darwin" && credentialStore !== "file") {
      if (!allowKeychain) {
        return makeUnavailableUsageLimits({
          checkedAt,
          reason: "unsupported",
          message: "Enable Cursor account usage in Kata Code to read its Keychain login.",
        });
      }
