...
import { cursorWorkspaceSlug } from "../acp/CursorPluginMcp.ts";
...
function assertSessionMcpKeepsT3CodeAndHostPlugins(
  ...
  assert.isTrue(names.includes("t3-code"), `${method} must include t3-code`);
  assert.isTrue(
    names.includes("plugin-linear-linear"),
    `${method} must include the Linear plugin server`,
  );
  assert.isTrue(
    names.includes("plugin-github-github"),
    `${method} must include other installed plugin servers`,
  );
  assert.notDeepEqual(
    names,
    ["t3-code"],
    `${method} must not exclusively replace host plugin servers with t3-code`,
  );
...
cursorAdapterTestLayer("CursorAdapterLive", (it) => {
  ...
  it.effect("sends session/cancel and settles the turn when sendTurn is interrupted", () =>
    Effect.gen(function* () {
      const adapter = yield* CursorAdapter;
      const serverSettings = yield* ServerSettingsService;
      const threadId = ThreadId.make("cursor-sendturn-interrupt");
      const tempDir = yield* Effect.promise(() =>
        NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "cursor-acp-")),
      );
      const requestLogPath = NodePath.join(tempDir, "requests.ndjson");
      ...
      const runtimeEventsFiber = yield* Stream.runForEach(adapter.streamEvents, (event) => {
        ...
        if (event.type === "turn.started") {
          return Deferred.succeed(turnStarted, undefined).pipe(Effect.ignore);
        }
        if (event.type === "turn.completed") {
      ...
      const sendTurnFiber = yield* adapter
        .sendTurn({
          threadId,
          input: "hang until the caller is interrupted",
          attachments: [],
        })
        .pipe(Effect.forkChild);
  ...
  it.effect(
    "does not cancel the active prompt approval when a queued sendTurn is interrupted",
    () =>
      Effect.gen(function* () {
        const adapter = yield* CursorAdapter;
        const serverSettings = yield* ServerSettingsService;
        const threadId = ThreadId.make("cursor-queued-sendturn-interrupt");
        const wrapperPath = yield* Effect.promise(() =>
          makeMockAgentWrapper({ T3_ACP_EMIT_TOOL_CALLS: "1" }),
        );
        yield* serverSettings.updateSettings({
        ...
        const runtimeEventsFiber = yield* Stream.runForEach(adapter.streamEvents, (event) => {
          ...
          if (event.type === "request.opened") {
            return Deferred.succeed(approvalOpened, event).pipe(Effect.ignore);
          }
          if (event.type === "request.resolved") {
        ...
        const opened = yield* Deferred.await(approvalOpened).pipe(Effect.timeout("5 seconds"));
        assert.equal(opened.type, "request.opened");

        const queuedTurnFiber = yield* adapter
          .sendTurn({
            threadId,
            input: "steer while the first prompt is waiting",
            attachments: [],
          })
          .pipe(Effect.forkChild);

        yield* Effect.sleep("300 millis");
        yield* Fiber.interrupt(queuedTurnFiber);

        const resolvedEarly = yield* Deferred.await(requestResolved).pipe(
          Effect.timeoutOption("300 millis"),
        );
        assert.isTrue(Option.isNone(resolvedEarly));
  ...
  it.effect("passes t3-code and Cursor plugin MCP servers on session/new and session/load", () => {
    const dataDir = NodeFS.mkdtempSync(
      NodePath.join(NodeOS.tmpdir(), "cursor-plugin-mcp-adapter-"),
    );
    ...
    const mcpSession = {
      environmentId: EnvironmentId.make("environment-plugin-mcp"),
      providerSessionId: "provider-session-plugin-mcp",
    ...
    const customAdapterLayer = Layer.effect(
      ...
      Layer.provideMerge(
        ServerConfig.layerTest(process.cwd(), {
          prefix: "t3code-cursor-adapter-plugin-mcp-",
        }),
      ),
      Layer.provideMerge(NodeServices.layer),
    );
    ...
    return Effect.gen(function* () {
      ...
      const serverSettings = yield* ServerSettingsService;
      const tempDir = yield* Effect.promise(() =>
        NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "cursor-acp-plugin-mcp-")),
      );
      const requestLogPath = NodePath.join(tempDir, "requests.ndjson");
      const argvLogPath = NodePath.join(tempDir, "argv.txt");
      yield* Effect.promise(() => NodeFSP.writeFile(requestLogPath, "", "utf8"));
      ...
      Effect.ensuring(
        Effect.sync(() => {
          McpProviderSession.clearMcpProviderSession(createThreadId);
          McpProviderSession.clearMcpProviderSession(loadThreadId);
        }),
      ),
      Effect.provide(customAdapterLayer),
    );
  });
