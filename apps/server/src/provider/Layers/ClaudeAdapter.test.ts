  ...
  it.effect("derives bypass permission mode from full-access runtime policy", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepEqual(createInput?.options.systemPrompt, {
        ...
        preset: "claude_code",
        append:
          "<runtime_info>In case you're asked: you are running in Kata Code through the Claude Code harness. No need to mention this otherwise. You can embed images and videos in your response using Markdown with absolute file paths.</runtime_info>",
      });
