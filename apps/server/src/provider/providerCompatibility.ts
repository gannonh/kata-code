...
export function resolveProviderCompatibility(
  ...
  const message =
    status === "broken"
      ? "This provider version is known to be incompatible with this Kata Code release."
      : status === "unsupported"
        ? "This provider version is outside the supported range for this Kata Code release."
        : status === "graceful"
          ? "This provider version has limited compatibility with this Kata Code release."
          : null;
