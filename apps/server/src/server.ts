...
import * as SpriteActivityLease from "./spriteActivityLease.ts";
...
const RoutineStoreLayerLive = RoutineStoreLive.pipe(Layer.provideMerge(SqlitePersistenceLayerLive));
...
const RuntimeCoreDependenciesWithoutRoutinesLive = ReactorLayerLive.pipe(
          ...
          Effect.gen(function* () {
              ...
              recoveryLock.withPermits(1)(
                Effect.gen(function* () {
                  ...
                  Effect.tap((recovered) =>
                    recovered
                      ? Effect.logInfo("Kata Code Connect managed tunnel recovered")
                      : Effect.void,
                  ),
                  Effect.catchCause((cause) =>
                    Cause.hasInterrupts(cause)
                      ? Effect.interrupt
                      : Effect.logWarning(
                          "Failed to recover the Kata Code Connect managed tunnel",
                          {
                            cause,
                          },
                        ),
            ...
            const wantsCliLink = hasCloudPublicConfig
              ? yield* CloudCliState.readCliDesiredCloudLink.pipe(
                  Effect.catch((cause) =>
                    Effect.logWarning("Failed to read the desired Kata Code Connect link", {
                      cause,
            ...
            const desiredCliLinkMode = wantsCliLink
              ? yield* CloudCliState.readCliDesiredLinkMode.pipe(
                  Effect.catch((cause) =>
                    Effect.logWarning("Failed to read the desired Kata Code Connect link mode", {
                      cause,
            ...
            const startedConfirmed =
              desiredCliLinkMode === "publish_only"
                ...
                : yield* startManagedCloudTunnelIfOriginConfirmed(localOrigin).pipe(
                    Effect.catch((cause) =>
                      Effect.logWarning("Failed to start the confirmed Kata Code Connect tunnel", {
                        cause,
                      }).pipe(Effect.as(false)),
                    ),
                  );
            const startStoredManagedTunnel = startManagedCloudTunnelIfOriginConfirmed(localOrigin, {
              ...
              Effect.tap((started) =>
                started
                  ? Effect.logWarning(
                      "Kata Code Connect started the stored tunnel without relay confirmation",
                    )
                  : Effect.void,
              ),
              Effect.catch((cause) =>
                Effect.logWarning("Failed to start the stored Kata Code Connect tunnel", { cause }),
              ),
              Effect.asVoid,
            );
            const registerManagedTunnel = retryManagedTunnelRegistration(
              ...
              Effect.tap((result) =>
                result.status === "ready"
                  ? Effect.logInfo("Kata Code Connect managed tunnel recovery registered")
                  : Effect.void,
              ),
              Effect.catchCause((cause) =>
                Cause.hasInterrupts(cause)
                  ? Effect.interrupt
                  : Effect.logWarning(
                      "Failed to register Kata Code Connect managed tunnel recovery",
                      {
                        cause,
                      },
            ...
            if (startupAction.action === "reconcile_link") {
              const reconciledMode = yield* reconcileDesiredCloudLinkIfStillDesired(
                ...
                Effect.catch((cause) =>
                  Effect.logWarning(
                    "Failed to reconcile Kata Code Connect desired link on startup",
                    {
                      cause,
                    },
                  ).pipe(Effect.as(null)),
