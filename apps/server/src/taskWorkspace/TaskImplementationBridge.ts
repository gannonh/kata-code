    ...
    resolve(scope).pipe(
      Effect.flatMap((invocation) =>
        taskWorkspaces.implementationAmendmentPropose({
          ...
          expectedTaskRevision: invocation.task.taskRevision,
          ...input,
          operationKey: `implementation-amendment:${invocation.occurrence.id}:${input.phaseId}:${input.workItemId}`,
        }),
      ),
    ...
    resolve(scope).pipe(
      Effect.flatMap((invocation) =>
        taskWorkspaces.implementationComplete({
          ...
          expectedTaskRevision: invocation.task.taskRevision,
          summary: input.summary,
          operationKey: `implementation-complete:${invocation.occurrence.id}`,
