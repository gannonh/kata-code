...
it.layer(
  ...
  it.effect("strips AppImage runtime env from terminal sessions", () =>
    Effect.gen(function* () {
      ...
      const { manager, ptyAdapter } = yield* createManager(5, {
        env: {
          APPIMAGE: "/home/user/Kata-Code.AppImage",
          APPDIR: appDir,
          ARGV0: "/home/user/Kata-Code.AppImage",
  ...
  it.effect("resolves a provider instance environment before spawning", () =>
    Effect.gen(function* () {
      ...
      const { manager, ptyAdapter } = yield* createManager(5, {
        env: { KATACODE_SECRET: "server-only" },
      ...
      expect(ptyAdapter.spawnInputs[0]?.env.KATACODE_SECRET).toBeUndefined();
