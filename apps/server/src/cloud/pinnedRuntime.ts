...
/**
 * A pinned runtime is an exact katacode release archive unpacked into
 ...
 * Node or npm on the machine; the only npm involvement in Kata Code is the `@kata-sh/code-cli`
 * package for people who prefer `npx katacode` or `npm install -g @kata-sh/code-cli`, and even a
 * CLI installed that way pins an archive when it sets up the service.
 */
const PINNED_RUNTIME_DIR = "runtime";
const PINNED_RUNTIME_INSTALL_TIMEOUT = Duration.minutes(10);
const PINNED_RUNTIME_ARCHIVE_FILE = "katacode-runtime-archive";
...
export function pinnedRuntimePaths(
  ...
  return {
    versionDir,
    entryPath: path.join(versionDir, platform === "win32" ? "katacode.exe" : "katacode"),
...
/**
 * Installs the katacode release archive for `version` into the pinned runtime
...
const installFromArchive = Effect.fn("cloud.pinned_runtime.install_archive")(function* (
  ...
  if (platformKey === undefined) {
    return yield* new PinnedRuntimeInstallError({
      step: `selecting a katacode release archive for ${input.platform}-${input.arch}`,
    });
  }
  ...
  const checksums = parseChecksums(
    new TextDecoder().decode(
      yield* fetchReleaseAsset(
        ...
        "downloading the katacode release checksums",
      ),
    ),
  );
  ...
  if (expected === undefined) {
    return yield* new PinnedRuntimeInstallError({
      step: `finding ${fileName} in the katacode release checksums`,
    });
  }
  const archive = yield* fetchReleaseAsset(
    httpClient,
    `${baseUrl}/${fileName}`,
    "downloading the katacode release archive",
    input.onProgress,
  );
  ...
  const digest = yield* Effect.tryPromise({
    try: () => crypto.subtle.digest("SHA-256", archive),
      ...
      new PinnedRuntimeInstallError({ step: "verifying the katacode release archive", cause }),
  });
  if (Encoding.encodeHex(new Uint8Array(digest)) !== expected) {
    return yield* new PinnedRuntimeInstallError({
      step: "verifying the katacode release archive checksum",
    });
  }
  ...
  const extractStep = "extracting the katacode release archive";
  // The archive wraps everything in one directory named after its stem;
  // strip it so the executable lands at <versionDir>/katacode.
  yield* input.runner
    .run({
...
const installPinnedRuntime = Effect.fn("cloud.pinned_runtime.ensure_installed")(function* (
  ...
  const ensureLegacyEntry = Effect.fn("cloud.pinned_runtime.ensure_legacy_entry")(
    function* (versionDir: string) {
      const legacyDir = input.path.join(versionDir, "node_modules", "@kata-sh", "code-cli", "dist");
