...
/**
 * A pinned runtime is an exact `@kata-sh/code-cli@<version>` npm-installed into
...
export function pinnedRuntimePaths(
  ...
  return {
    versionDir,
    entryPath: path.join(versionDir, "node_modules", "@kata-sh", "code-cli", "dist", "bin.mjs"),
...
/**
 * Installs `@kata-sh/code-cli@<version>` into the pinned runtime directory
...
const installPinnedRuntime = Effect.fn("cloud.pinned_runtime.ensure_installed")(function* (
  ...
  return yield* Effect.gen(function* () {
    const installStep = "installing the pinned Kata Code runtime (this can take a few minutes)";
    yield* runner
      .run({
        command: "npm",
        args: [
          ...
          "--no-fund",
          "--no-audit",
          `@kata-sh/code-cli@${input.version}`,
        ],
