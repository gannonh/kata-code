...
describe("releaseManagedTunnelOnShutdown", () => {
  ...
  it.effect("still releases a pending update when the launcher is stopping", () => {
    // `katacode service uninstall` or `systemctl stop` during the pending window:
  ...
  it.effect("recovers a web-linked tunnel with its environment credential", () => {
    ...
    return Effect.gen(function* () {
      ...
      // Routine webhooks follow the public origin of the recovered tunnel.
      expect(new TextDecoder().decode(values.get(CLOUD_MANAGED_ENDPOINT_URL))).toBe(
