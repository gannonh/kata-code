...
const version = "1.2.3";
const archiveName = `katacode-${version}-linux-x64.tar.gz`;
...
it.layer(NodeServices.layer)("ensurePinnedRuntimeInstalled", (it) => {
  it.effect("installs the verified release archive as the runtime executable", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "katacode-pinned-archive-" });
      ...
      assert.equal(paths.entryPath, path.join(paths.versionDir, "katacode"));
      ...
      assert.isFalse(yield* fs.exists(path.join(paths.versionDir, "katacode-runtime-archive")));
      if ((yield* HostProcessPlatform) !== "win32") {
        ...
        const legacyStart = yield* runner.run({
          ...
          args: [
            path.join(paths.versionDir, "node_modules/@kata-sh/code-cli/dist/bin.mjs"),
            "serve",
          ],
        });
  ...
  it.effect("refuses an archive whose checksum does not match the release", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "katacode-pinned-archive-bad-" });
      const commands: string[] = [];
      ...
      assert.equal(error.step, "verifying the katacode release archive checksum");
