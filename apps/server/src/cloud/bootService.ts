...
import { CLI_RELEASE_BASE_URL_ENV } from "@kata-sh/code-shared/cliRelease";
...
/** File in the logs dir that receives the service's stdout and stderr. `katacode triage` points agents at it. */
...
/**
 * Reads `KATACODE_HOME` back out of a rendered unit or plist. Only values this
...
export function bootServiceBaseDirOf(contents: string): string | undefined {
  const systemd = /^Environment=KATACODE_HOME=(.*)$/m.exec(contents)?.[1];
  ...
  const plist = /<key>KATACODE_HOME<\/key>\s*<string>([^<]*)<\/string>/.exec(contents)?.[1];
  if (plist !== undefined) {
...
export function formatBootServiceProblem(problem: BootServiceProblem): string {
  switch (problem) {
    case "user-manager-unavailable":
      return "Cannot reach the systemd user manager. Run `systemctl --user status` in a login session for the service user. Install your distribution's systemd user-session support if it is missing; do not run Kata Code with sudo.";
    ...
    case "linger-disabled":
      return 'Lingering is disabled. Kata Code will stop when your last login session ends and will not start at boot. Run `sudo loginctl enable-linger "$(id -un)"` on this machine, then retry the service command as your normal user.';
    case "service-disabled":
      return "The service is not enabled to start automatically. Run `katacode service install` to repair it.";
    case "service-stopped":
      return "The service is not running. Check the service log and `systemctl --user status t3code.service`, then run `katacode service install`.";
    case "restart-pending":
      return "A newer version is installed but the service is still running the previous one. Run `katacode service restart` to switch.";
  }
}
...
export class BootServiceDowngradeRefusedError extends Schema.TaggedError<BootServiceDowngradeRefusedError>()(
  ...
  override get message(): string {
    return `Refusing to replace katacode@${this.installedVersion} with older katacode@${this.targetVersion}. Run the command again with --allow-downgrade to continue.`;
  }
}
...
export interface BootServiceStatus {
  ...
  /**
   * The Kata Code home the installed unit serves. The unit name is fixed per user,
   * so a caller working against another base dir must not treat this service
   * as its own; `katacode update --base-dir` learned that by restarting the live
...
export class BootService extends Context.Service<
  BootService,
  {
    readonly install: (options?: {
       ...
       * is running now. `katacode update` uses this when the user declines the
       * restart, so a later `katacode service restart` lands on the new version.
       */
...
>()("@kata-sh/code-cli/cloud/bootService") {}
            ...
            Effect.mapError(
              (cause) =>
                new PinnedRuntimeInstallError({
                  step: "verifying the pinned Kata Code runtime",
                  cause,
                }),
            ),
