...
const linuxRuntime = "/home/theo/.katacode/runtime/versions/1.2.3/katacode";
const linuxPlan = {
  program: [linuxRuntime, "__service-launcher"],
  baseDir: "/home/theo/.katacode",
  logPath: "/home/theo/.katacode/userdata/logs/boot-service.log",
...
it("reads the served Kata Code home back out of a rendered unit or plist", () => {
  const plan = (baseDir: string) => ({
    program: [`${baseDir}/runtime/versions/1.2.3/katacode`, "__service-launcher"],
    baseDir,
  ...
  expect(
    BootService.bootServiceBaseDirOf(
      BootService.renderBootServiceUnit(plan("/home/theo/.katacode")),
    ),
  ).toBe("/home/theo/.katacode");
...
const macRuntime = "/Users/theo/.katacode/runtime/versions/1.2.3/katacode";
const macPlan = {
  program: [macRuntime, "__service-launcher"],
  baseDir: "/Users/theo/.katacode",
  logPath: "/Users/theo/.katacode/userdata/logs/boot-service.log",
...
it("appends both stdio streams to the boot service log", () => {
  ...
  expect(plist).toContain(
    "<key>StandardOutPath</key>\n  <string>/Users/theo/.katacode/userdata/logs/boot-service.log</string>",
  );
  expect(plist).toContain(
        ...
        stdout:
          input.args[0] === "--version"
            ? // The runtime under test reports the version of the directory it
              ...
              `katacode v${/versions\/([^/]+)\//.exec(input.command)?.[1] ?? "1.2.3"}\n`
  ...
  const makeService = (
    ...
    Effect.gen(function* () {
      ...
      return yield* BootService.make({
        ...
        cliVersion,
        host: { execPath: "/usr/bin/katacode" },
      });
    }).pipe(
      ...
      Effect.provide(
        Layer.mergeAll(
          ...
          Layer.succeed(HostProcessExecutablePath, "/usr/bin/katacode"),
          Layer.succeed(
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  ...
  it.effect("install with start=false rewrites the files and marks a restart pending", () =>
    Effect.gen(function* () {
      ...
      expect(yield* fs.readFileString(plan.unitPath)).toContain("versions/1.2.4/katacode");
      expect(
        commands.filter(
      ...
      // the reason is named so `katacode service status` can point at restart.
  ...
  it.effect("restart leaves a service that serves another Kata Code home alone", () =>
    Effect.gen(function* () {
      ...
      const other = yield* makeService(undefined, "1.2.3", path.join(otherHome, ".katacode"));
