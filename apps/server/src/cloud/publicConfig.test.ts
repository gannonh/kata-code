  ...
  Effect.gen(function* () {
    assert.equal(
      yield* hostedAppUrlConfig.pipe(
        provideEnv({ KATACODE_HOSTED_APP_URL: "https://nightly.app.kata.sh" }),
      ),
      "https://nightly.app.kata.sh",
    );
    assert.equal(
      yield* hostedAppUrlConfig.pipe(
        provideEnv({ KATACODE_HOSTED_APP_URL: "http://localhost:5733" }),
      ),
      "http://localhost:5733",
    );
  }),
...
it.effect("rejects malformed or insecure hosted app URLs", () =>
  Effect.gen(function* () {
    for (const value of [
      "app.kata.sh",
      "http://app.kata.sh",
      "https://app.kata.sh/nested",
      "https://app.kata.sh?alias=true",
    ]) {
      const result = yield* hostedAppUrlConfig.pipe(
        provideEnv({ KATACODE_HOSTED_APP_URL: value }),
        Effect.result,
      );
