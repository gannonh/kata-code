...
describe("ProviderRuntimeIngestion", () => {
  ...
  it("clears lastError when a later turn completes successfully", async () => {
    ...
    harness.emit({
      type: "turn.started",
      eventId: asEventId("evt-turn-started-failed-then-ok"),
      provider: ProviderDriverKind.make("codex"),
      threadId: asThreadId("thread-1"),
      createdAt: now,
      turnId: asTurnId("turn-fail-1"),
    });

    await waitForThread(
      harness.readModel,
      (thread) =>
        thread.session?.status === "running" && thread.session?.activeTurnId === "turn-fail-1",
    );

    harness.emit({
      type: "turn.completed",
      eventId: asEventId("evt-turn-completed-failed-then-ok"),
      provider: ProviderDriverKind.make("codex"),
      threadId: asThreadId("thread-1"),
    ...
    await waitForThread(
      harness.readModel,
      (entry) => entry.session?.status === "error" && entry.session?.lastError === "turn failed",
    );

    harness.emit({
      type: "turn.started",
      eventId: asEventId("evt-turn-started-recovery"),
      provider: ProviderDriverKind.make("codex"),
      threadId: asThreadId("thread-1"),
      createdAt: "2026-01-01T00:00:02.000Z",
      turnId: asTurnId("turn-ok-1"),
    });

    await waitForThread(
      harness.readModel,
      (thread) =>
        thread.session?.status === "running" && thread.session?.activeTurnId === "turn-ok-1",
    );

    harness.emit({
      type: "turn.completed",
      eventId: asEventId("evt-turn-completed-recovery"),
      provider: ProviderDriverKind.make("codex"),
      threadId: asThreadId("thread-1"),
  ...
  it("maps turn aborted events into ready session state", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
  ...
  it("ignores reasoning-only content deltas and surfaces completion detail when present", async () => {
    ...
    const thread = await waitForThread(harness.readModel, (entry) =>
      entry.messages.some(
        (message: ProviderRuntimeTestMessage) =>
          message.id === "assistant:item-reasoning" && !message.streaming,
      ),
    );
    const message = thread.messages.find(
