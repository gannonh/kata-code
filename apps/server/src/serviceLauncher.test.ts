...
// A pinned runtime is an executable at <versionDir>/katacode. The tests stand one up
...
const writeFakeRuntime = (
  ...
  Effect.gen(function* () {
    const entryPath = path.join(versionDir, "katacode");
...
it.layer(NodeServices.layer)("service state persistence", (it) => {
  it.effect("durably replaces and strictly reads one state document", () =>
    Effect.gen(function* () {
      ...
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "katacode-service-launcher-test-" });
  ...
  it.effect("a fresh launcher clears a restart deferred by katacode update", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const root = yield* fs.makeTempDirectoryScoped({
        prefix: "katacode-service-launcher-restart-",
      });
      const statePath = path.join(root, "runtime", "service-state.json");
  ...
  it.effect("serializes shutdown with launcher recovery", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "katacode-service-launcher-stop-" });
      const statePath = path.join(root, "runtime", "service-state.json");
      yield* writeFakeRuntime(
        fs,
        path,
  ...
  it.effect("commits only after the trial reports prepared", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "katacode-service-launcher-flow-" });
      const statePath = path.join(root, "runtime", "service-state.json");
...
`;
  ...
  it.effect("rolls back a trial that reports the wrong update ID", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fs.makeTempDirectoryScoped({
        prefix: "katacode-service-launcher-rollback-",
      });
      const statePath = path.join(root, "runtime", "service-state.json");
      const databasePath = path.join(root, "userdata", "state.sqlite");
      yield* fs.makeDirectory(path.dirname(databasePath), { recursive: true });
...
`;
  ...
  it.effect("restores the database when a migrating trial exits", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "katacode-service-launcher-db-" });
      const statePath = path.join(root, "runtime", "service-state.json");
      const databasePath = path.join(root, "userdata", "state.sqlite");
