...
import { HostProcessUserId } from "@kata-sh/code-shared/hostProcess";
...
/**
 * Serves `katacode app` requests on the local control address until `close`.
 *
  ...
  const handleConnection = (socket: NodeNet.Socket) => {
    ...
    socket.on("data", (chunk) => {
      ...
      void input.handle(parsed).then(finish, () => {
        finish(
          invalidResponse(parsed.requestId, "Kata Code could not process the desktop app request."),
        );
      });
    });
...
>()("@kata-sh/code-desktop/app/DesktopAppActivation") {}
