...
const makeDesktopClerkLayer = (isDevelopment = true, events: string[] = []) => {
  const environment = DesktopEnvironment.DesktopEnvironment.of({
    ...
    userDataDirName: isDevelopment ? "katacode-dev" : "t3code",
    legacyUserDataDirName: isDevelopment ? "Kata Code (Dev)" : "Kata Code (Alpha)",
...
describe("DesktopClerk", () => {
  ...
  it.effect("acquires and releases the SDK bridge with the layer", () => {
    ...
    return Effect.gen(function* () {
        ...
        [
          {
            ...
            passkeys: true,
            renderer: { scheme: "katacode-dev", host: "app" },
          },
        ],
      ]);
      ...
      assert.deepEqual(events, ["setPath:userData:/tmp/app-data/katacode-dev", "createClerkBridge"]);
  ...
  it.each([
    { isDevelopment: true, scheme: "katacode-dev" },
    { isDevelopment: false, scheme: "katacode" },
