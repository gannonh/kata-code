...
<header>↑ Drag Kata Code into the list above</header>
<button id="app" draggable="true" aria-label="Drag Kata Code to System Settings, or click to reveal in Finder"><img src="${escapeHtml(icon)}" alt="" draggable="false">Kata Code</button>
</main></body></html>`;
}
...
export class MacPermissionHelper {
  ...
  async show(
    ...
    if (!appIcon) throw new Error("The packaged Kata Code icon is missing.");
