...
vi.mock("electron", async () => {
  ...
  return {
    app: {
      getPath: () => "/Applications/Kata Code (Nightly).app/Contents/MacOS/Kata Code",
    },
...
describe("macAppBundlePath", () => {
  it("resolves bundles with spaces and refuses non-bundle executables", () => {
    expect(macAppBundlePath("/Applications/Kata Code.app/Contents/MacOS/Kata Code")).toBe(
      "/Applications/Kata Code.app",
    );
    expect(macAppBundlePath("/usr/local/bin/electron")).toBeUndefined();
    expect(macAppBundlePath("/Applications/Kata Code.app/other/MacOS/Kata Code")).toBeUndefined();
  });
});
it("drags the running app bundle only for the helper's own renderer", async () => {
  ...
  expect(mocks.startDrag).toHaveBeenCalledWith({
    file: "/Applications/Kata Code (Nightly).app",
    icon: mocks.createFromPath.mock.results[0]!.value.resize(),
  });
  send("finder");
  expect(mocks.showItemInFolder).toHaveBeenCalledWith("/Applications/Kata Code (Nightly).app");
});
...
it("does not show a helper with a missing packaged icon", async () => {
  ...
  await expect(open()).rejects.toThrow("packaged Kata Code icon is missing");
  expect(windows).toHaveLength(0);
});
