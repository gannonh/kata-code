    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        DesktopConfig.layerTest({
          ...
          KATACODE_OTLP_TRACES_URL: options?.otlpTracesUrl,
          KATACODE_OTLP_METRICS_URL: options?.otlpMetricsUrl,
          KATACODE_OTLP_LOGS_URL: options?.otlpLogsUrl,
        }),
      ),
    ),
  );
...
describe("DesktopBackendConfiguration", () => {
    ...
    () =>
      Effect.gen(function* () {
        ...
        const standard = {
          ...
          KATACODE_OTLP_TRACES_URL: "http://t3.example.com:4318/v1/traces",
        };
        const previousWslEnv = process.env.WSLENV;
        // A developer's own OTLP variables would be forwarded too.
        const ambientOtel = Object.entries(process.env).filter(
          ([name]) => name.startsWith("OTEL_") || name.startsWith("KATACODE_OTLP_"),
        );
        try {
          ...
          yield* Effect.gen(function* () {
            ...
            assert.equal(
              config.env.KATACODE_OTLP_TRACES_URL,
              "http://t3.example.com:4318/v1/traces",
            );
            assert.include(wslEnv, "KATACODE_OTLP_TRACES_URL");
          }).pipe(
            Effect.provide(
              DesktopBackendConfiguration.layer.pipe(
                Layer.provideMerge(serverExposureLayer),
  ...
  it.effect("resolveWsl preserves existing WSLENV entries when forwarding backend secrets", () =>
    Effect.gen(function* () {
      ...
      const previousOtlpHeaders = process.env.KATACODE_OTLP_HEADERS;
      const previousOtlpProtocol = process.env.KATACODE_OTLP_PROTOCOL;
      ...
      try {
        ...
        process.env.KATACODE_OTLP_HEADERS = 'authorization="Bearer%20my-token"';
        process.env.KATACODE_OTLP_PROTOCOL = "http/protobuf";
        ...
        yield* Effect.gen(function* () {
          ...
          assert.equal(config.env.KATACODE_OTLP_PROTOCOL, "http/protobuf");
          ...
          assert.equal(
            config.env.WSLENV,
            "GOPATH/p:OPENAI_API_KEY/u:EMPTY::AZURE_DEVOPS_EXT_PAT/u:ANTHROPIC_API_KEY:KATACODE_OTLP_HEADERS:KATACODE_OTLP_PROTOCOL",
          );
        }).pipe(
          Effect.provide(
            DesktopBackendConfiguration.layer.pipe(
        ...
        restoreEnv("KATACODE_OTLP_HEADERS", previousOtlpHeaders);
        restoreEnv("KATACODE_OTLP_PROTOCOL", previousOtlpProtocol);
        for (const [name, value] of ambientOtel) restoreEnv(name, value);
      }
    }).pipe(Effect.scoped, Effect.provide(NodeServices.layer)),
  );
  ...
  it("resolvePrimaryLabel is runSync-safe against the real WSL availability probe", async () => {
    ...
    // oxlint-disable-next-line kata-code/no-manual-effect-runtime-in-tests -- This test intentionally replicates the sync IPC handler's runSync path to catch a regression to async-only resolution; it.effect would mask it.
    ...
    try {
      ...
      // oxlint-disable-next-line kata-code/no-manual-effect-runtime-in-tests -- Same reason: this is the synchronous resolution the IPC handler performs.
