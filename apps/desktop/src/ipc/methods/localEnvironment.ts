...
export const setLocalEnvironmentEnabled = makeIpcMethod({
  ...
  handler: Effect.fn("desktop.ipc.localEnvironment.setEnabled")(function* (enabled) {
    ...
    const setEnabled = appSettings.setLocalEnvironmentEnabled;
    if (setEnabled === undefined) {
      return yield* Effect.die(new Error("Desktop settings do not support local environments"));
    }
    const change = yield* setEnabled(enabled);
    if (change.changed) {
