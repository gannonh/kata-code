...
// Stands in for the release's self-contained `katacode` executable: the install
...
describe("WSL runtime cache", () => {
  ...
  it("installs through a temporary directory and only reuses valid completed caches", () => {
    const script = buildWslRuntimeInstallScript(
      "/mnt/c/Program Files/Kata Code/wsl-runtime.tar.gz",
      "1.2.3-x64",
      "b".repeat(64),
    );

    expect(script).toContain('runtime_parent="$HOME/.katacode/wsl-runtime"');
    ...
    // The release archive wraps everything in one `katacode-<version>-linux-x64/`
    // directory; stripping it puts the executable at `$runtime_root/katacode`.
    expect(script).toContain(
      "tar -xzf '/mnt/c/Program Files/Kata Code/wsl-runtime.tar.gz' -C \"$runtime_tmp\" --strip-components=1",
    );
  ...
  it("verifies the archive digest before extracting, and only on a cache miss", () => {
    ...
    expect(script).toContain(
      "archive_sha=$(sha256sum '/mnt/c/Program Files/Kata Code/wsl-runtime.tar.gz' | cut -d ' ' -f 1)",
    );
  ...
  it("treats a runtime whose executable no longer runs as a cache miss", () => {
    ...
    expect(script).toContain('  [ -x "$1/katacode" ] && "$1/katacode" --version >/dev/null 2>&1');
  ...
  // A swapped or half-written `katacode` can still exist and even still answer
  ...
  it("re-hashes the executable against the digest the install recorded", () => {
    ...
    expect(script).toContain(`  sha256sum "$1/katacode" 2>/dev/null | cut -d ' ' -f 1`);
    expect(script).toContain(
  ...
  it("parses only absolute Linux runtime paths", () => {
    expect(parseWslRuntimeRoot("runtimeRoot:/home/josh/.katacode/wsl-runtime/1.2.3-x64\n")).toBe(
      "/home/josh/.katacode/wsl-runtime/1.2.3-x64",
    );
  ...
  it("never deletes a runtime another backend is running from", () => {
    ...
    // The running backend's argv holds `<runtime>/katacode`, so
  ...
  it("invalidates a cache by dropping its ready marker, not the tree", () => {
    ...
    expect(script).toContain(
      'rm -f "$HOME/.katacode/wsl-runtime/1.2.3_x64/.t3code-wsl-runtime-ready"',
    );
  ...
  const createFixture = () => {
    const result = runShell(
      [
        ...
        'stage="$work/stage/katacode-0.0.0-linux-x64"',
        'mkdir -p "$stage/node_modules/node-pty/build/Release" "$work/home"',
        `printf '%s' ${sh(SERVER_ENTRY_SOURCE)} > "$stage/katacode"`,
        'chmod +x "$stage/katacode"',
        `printf '%s' 'pty-native-payload' > "$stage/node_modules/node-pty/build/Release/pty.node"`,
        `tar -czf "$work/wsl-runtime.tar.gz" -C "$work/stage" katacode-0.0.0-linux-x64`,
        `printf 'work:%s\\n' "$work"`,
    ...
    return {
      ...
      archiveSha,
      runtimeId,
      runtimeParent: `${work}/home/.katacode/wsl-runtime`,
      runtimeRoot: `${work}/home/.katacode/wsl-runtime/${runtimeId}`,
      serverEntry: `${work}/home/.katacode/wsl-runtime/${runtimeId}/katacode`,
      installScript,
  ...
  it("extracts once when two installs race for the same cache", () => {
    ...
    const raced = runShell(
      [
        ...
        'PATH="$work/bin:$PATH"',
        "export PATH",
        `cat > "$work/install.sh" <<'KATACODE_INSTALL_SCRIPT'`,
        fixture.installScript(),
        "KATACODE_INSTALL_SCRIPT",
  ...
  it("leases a warm cache across the prepare-to-spawn handoff", () => {
    ...
    const selected = runShell(
      [
        ...
        `cat > ${sh(`${fixture.work}/select.sh`)} <<'KATACODE_SELECT_SCRIPT'`,
        fixture.installScript(),
        "KATACODE_SELECT_SCRIPT",
  ...
  it("removes an aged stale tree after replacing an active unready cache", () => {
    ...
    const result = runShell(
      [
        ...
        'sh -c "sleep 30; :" "$runtime_root/katacode" >/dev/null 2>&1 &',
        "active_pid=$!",
        "sleep 0.1",
        fixture.installScript(),
  ...
  it("prunes old and markerless caches without touching retained, active, locked, or unrelated roots", () => {
    const result = runShell(
      [
        ...
        "work=$(mktemp -d)",
        'home="$work/home"',
        'runtime_parent="$home/.katacode/wsl-runtime"',
        ...
        'sh -c "sleep 30; :" "$runtime_parent/sha256-active/katacode" >/dev/null 2>&1 &',
        "active_pid=$!",
        "(",
