...
describe("BrowserSession", () => {
  ...
  it.effect("keeps scopes that differ only by a lone surrogate in separate partitions", () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(
        ...
        "persist:katacode-preview-f051bb2c68cb7b2fe969",
      );
    }).pipe(Effect.provide(layer)),
  );
  ...
  it.effect("keeps legacy defaults disjoint from nondefault profile partitions", () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(legacyDefault, "persist:katacode-preview-78f0be89237d77f7a70e");
      assert.strictEqual(
        nondefaultProfile,
        "persist:katacode-preview-profile-78f0be89237d77f7a70e",
      );
