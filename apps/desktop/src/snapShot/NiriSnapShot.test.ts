...
it("does not activate T3 until requested, then matches PID and title", async () => {
  ...
  windows = [
    { ...window, id: 1, pid: 999, title: "Kata Code" },
    { ...window, id: 2, pid: process.pid, title: "Other T3" },
    { ...window, id: 3, pid: process.pid, title: "Kata Code" },
  ];
  await snapshot.feedback!.activate("Kata Code");
    ...
    if (request === "EventStream")
      send(socket, {
        WindowOpenedOrChanged: {
          window: { ...window, id: 4, pid: process.pid, title: "Kata Code" },
        },
      });
  };
  await snapshot.feedback!.activate("Kata Code");
...
it("rejects ambiguous activation targets", async () => {
  const snapshot = await captureNiriWindow(socketPath);
  windows = [1, 2].map((id) => ({ ...window, id, pid: process.pid, title: "Kata Code" }));
  await expect(snapshot.feedback!.activate("Kata Code")).rejects.toThrow("More than one");
});
...
it("cancels pending activation when capture feedback is closed", async () => {
  ...
  const activation = expect(snapshot.feedback!.activate("Kata Code")).rejects.toThrow("cancelled");
