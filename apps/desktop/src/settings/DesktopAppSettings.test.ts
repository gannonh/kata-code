    ...
    () =>
      withSettings(
        Effect.gen(function* () {
          ...
          const setLocalEnvironmentEnabled = settings.setLocalEnvironmentEnabled;
          assert.isDefined(setLocalEnvironmentEnabled);
          if (setLocalEnvironmentEnabled === undefined) return assert.fail();
          assert.isTrue((yield* setLocalEnvironmentEnabled(false)).changed);
          assert.deepEqual(yield* settings.load, { ...before, localEnvironmentEnabled: false });
          assert.isFalse((yield* setLocalEnvironmentEnabled(false)).changed);
          yield* setLocalEnvironmentEnabled(true);
