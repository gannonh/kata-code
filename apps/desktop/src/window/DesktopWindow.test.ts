...
import { DesktopSnapShotId } from "@kata-sh/code-contracts";
...
describe("DesktopWindow", () => {
  ...
  it.effect("reveals a macOS window when did-finish-load beats ready-to-show", () =>
    Effect.gen(function* () {
      const fakeWindow = makeFakeBrowserWindow();
      const createCount = yield* Ref.make(0);
      const mainWindow = yield* Ref.make<Option.Option<Electron.BrowserWindow>>(Option.none());
  ...
  it.effect("delivers capture completion and late errors without taking focus back", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        assert.equal(foreground, "Kata Code");
        foreground = "Explorer";
