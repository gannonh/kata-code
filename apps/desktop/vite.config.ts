...
export default defineConfig({
  run: {
    tasks: {
      ...
      dev: {
        command:
          "node scripts/build-browser-secret.mjs && node scripts/build-preview-annotation-css.mjs && cross-env KATACODE_DESKTOP_DEV=1 vp pack --watch",
        dependsOn: ["@kata-sh/code-cli#build"],
        cache: false,
      },
      "dev:bundle": {
  ...
  pack: [
    {
      ...
      clean: true,
      deps: {
        alwaysBundle: (id) => id.startsWith("@kata-sh/code-"),
      },
