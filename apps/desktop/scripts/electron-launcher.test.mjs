...
describe("electron development launcher", () => {
  ...
  it("keeps the launcher script free of volatile environment values", () => {
    ...
    assert.include(
      script,
      "exec '/repo/node_modules/electron/Electron' --katacode-dev-root='/repo/apps/desktop' '/repo/apps/desktop/dist-electron/main.cjs' \"$@\"",
    );
  });
  ...
  it("keeps the native Electron executable name inside the branded macOS bundle", () => {
    const paths = resolveMacLauncherPaths(
      "/repo/apps/desktop/.electron-runtime/Kata Code (Dev).app",
      "Kata Code (Dev)",
    );

    assert.equal(paths.launcherExecutableName, "Kata Code (Dev) Launcher");
    assert.equal(
      paths.launcherBinaryPath,
      "/repo/apps/desktop/.electron-runtime/Kata Code (Dev).app/Contents/MacOS/Kata Code (Dev) Launcher",
    );
    assert.equal(
      paths.runtimeElectronBinaryPath,
      "/repo/apps/desktop/.electron-runtime/Kata Code (Dev).app/Contents/MacOS/Electron",
    );
    ...
    assert.include(
      script,
      "exec '/repo/apps/desktop/.electron-runtime/Kata Code (Dev).app/Contents/MacOS/Electron'",
    );
    assert.notInclude(script, "node_modules/electron");
  });

  it("declares why the macOS app needs protected access", () => {
    const values = resolveMacBundleInfoPlistStrings("Kata Code (Dev) Launcher");
    ...
    assert.equal(
      ...
      "Kata Code reads project files you open in the desktop app.",
    );
  });

  it("ad-hoc signs the complete development app bundle", () => {
    assert.deepEqual(resolveMacCodeSignArguments("/runtime/Kata Code (Dev).app"), [
      "--force",
      "--deep",
      "--sign",
      "-",
      "--timestamp=none",
      "/runtime/Kata Code (Dev).app",
    ]);
  });
  ...
  it("derives launcher icons from canonical development and production assets", () => {
    ...
    assert.match(development.sourceIconPath, /apps\/desktop\/resources\/source\.png$/);
    assert.equal(development.generatedIconPath, "/runtime/icon-dev.icns");
    assert.match(production.sourceIconPath, /apps\/desktop\/resources\/source\.png$/);
