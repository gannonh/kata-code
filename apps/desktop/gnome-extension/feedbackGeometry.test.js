...
it("activates only a window of the process authenticated by D-Bus", () => {
  const target = window(42, "Kata Code");
  expect(findCaptureDestination([window(99, "Kata Code"), target], 42, "Kata Code")).toBe(target);
  expect(findCaptureDestination([window(99, "Kata Code")], 42, "Kata Code")).toBeUndefined();
