...
>()("kata-code-relay/db/RelayTransactions") {
  ...
  const database =
    mode === "shared-database"
      ? yield* Planetscale.PostgresDatabase("RelayPostgresDatabase", {
          name: "katacoderelay",
          region: { slug: "us-west" },
          // Single-node PS-5 is the $5 plan. A larger clusterSize or replicas
          // above 0 raises the production bill on the next relay deploy.
          clusterSize: "PS_5",
          arch: "arm",
          replicas: 0,
