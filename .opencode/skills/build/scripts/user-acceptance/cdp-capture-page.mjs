...
if (!evidenceDir || !screenshotPath || !["starting", "key", "final"].includes(checkpoint)) {
  console.error(
    "Usage: cdp-capture-page.mjs --evidence <dir> --cdp http://127.0.0.1:9222 [--title <title>|--url-contains <text>] --screenshot <relative-path> --checkpoint <starting|key|final> [--text <relative-path>]",
  );
  process.exit(2);
}
...
if (existsSync(join(evidenceDir, "evidence.json"))) {
  ...
  manifest.artifacts.push({
    ...
    description: `CDP ${checkpoint} screenshot for ${target.title}`,
  });
  if (resolvedTextPath)
    manifest.artifacts.push({
      type: "log",
