...
function isLinearIssueUrl(value) {
  try {
    ...
    return (
      ...
      /\/issue\/[A-Z0-9]+-\d+/i.test(url.pathname)
    );
  } catch {
    return false;
  }
...
if (manifest) {
  ...
  if (!["user-facing", "technical-enablement"].includes(manifest.mode))
    fail(`invalid evidence mode: ${manifest.mode}`);
  ...
  if (!Array.isArray(manifest.slices) || manifest.slices.length === 0)
    fail("slices must be a non-empty array");
  ...
  for (const command of commands) {
    ...
    if (!command.output_path || !fileIsNonempty(command.output_path))
      fail(`command output is missing or empty: ${command.output_path ?? "<missing>"}`);
  }
  ...
  for (const slice of slices) {
    ...
    if (!SLICE_RESULTS.includes(slice.result))
      fail(`slice ${label} has invalid result: ${slice.result}`);
  ...
  const hasRequiredCommand = commands.some(
    (command) => command.kind === requiredKind && command.exit_code === 0,
  );
  if (!hasRequiredCommand)
    fail(`${manifest.mode} evidence needs a passing captured command with kind ${requiredKind}`);
  ...
  if (manifest.visual === true) {
    ...
    const startingPaths = new Set(
      screenshots
        .filter((artifact) => artifact.checkpoint === "starting")
        .map((artifact) => artifact.path),
    );
    const hasDistinctFinal = screenshots.some(
      (artifact) => artifact.checkpoint === "final" && !startingPaths.has(artifact.path),
    );
    ...
    const videoSkipPattern =
      /^Video:\s*Skipped\s*[—-]\s*.+;\s*attempted:\s*.+;\s*suggested tooling:\s*.+$/i;
    const hasVideoSkipped = (manifest.notes ?? []).some((note) => videoSkipPattern.test(note));
    if (!hasVideo && !hasVideoSkipped) {
      fail(
        "visual evidence needs a video artifact or `Video: Skipped — <reason>; attempted: <tool>; suggested tooling: <tooling>`",
      );
    }
  }
}
