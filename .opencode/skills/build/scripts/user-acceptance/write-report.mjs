...
if (manifest.visual === true) {
  screenshotStatus =
    hasStartingScreenshot && hasFinalScreenshot
...
if (manifest.visual === true) {
  if (videos.length > 0)
    videoStatus = `Captured - ${videos.map((artifact) => `\`${artifact.path}\``).join(", ")}`;
...
lines.push(
  `- ${requiredKind === "e2e" ? "E2E" : "Contract"}: ${requiredCommand ? `Pass - \`${requiredCommand.output_path}\`` : "Blocked - no passing captured command"}`,
);
