...
2. **Triage in priority order.**
   ...
   - Failing checks (`statusCheckRollup` entries with `conclusion: FAILURE`): an Actions entry's `detailsUrl` is `https://github.com/<owner>/<repo>/actions/runs/<run-id>/job/<job-id>`; take the run ID from it, or resolve it with `gh run list`, then pull logs with `gh run view <run-id> --log-failed`. Root-cause the failure; fix the underlying code or test; commit; push.
