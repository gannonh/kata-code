...
Each project uses `text.svg` for its app mark and `background.svg` when the background is a vector layer. The nightly mark is the yellow Kata logo; development and legacy source layers may retain their historical artwork. Additional layers use semantic names that describe their role and placement.
