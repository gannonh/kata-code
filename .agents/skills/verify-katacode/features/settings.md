...
- **Nav.** Choose `General` in the settings sidebar. The breadcrumb returns to `General`. Choose `Appearance`. The breadcrumb returns to `Appearance`. The environment-scope nav lists `General`, `Appearance`, `Keybindings`, `SnapShots`, `Providers`, `Integrations`, `Source Control`, `Connections`, and `Archive`. Project or checkout scope also shows the singular item `Project`, which routes to `/settings/projects`. `Archive` routes to `/settings/archived`. `SnapShots` routes to `/settings/snap-shot`. On web, SnapShots shows `Only available in the desktop app.` That is enough. Do not treat an absent `Project` item at environment scope or disabled capture as a missing Settings page.
