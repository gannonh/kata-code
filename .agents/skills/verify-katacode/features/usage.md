...
- **Wait for totals.** The page shows a skeleton until the environment answers. Wait until heading `Totals` is present. Do not assert numbers on the skeleton.
- **Empty window.** If this machine has no provider usage transcripts, the large figure is `$0.00`, the caption includes `0 sessions`, and the breakdown table says `No activity in this window.` If Codex/Claude/other CLIs on this host have usage, the page shows those totals even on a disposable Kata home. That is expected. Non-zero numbers are not proof that you attached to `~/.katacode`.
...
- Narrow viewports replace the segmented `Usage metric` / `Usage period` groups with compact selects that still use those aria-labels. Prefer a desktop width.
