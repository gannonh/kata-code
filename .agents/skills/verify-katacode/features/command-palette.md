...
- **Open settings.** From the empty-query `Actions` list, activate `Open settings`. Typing the two-word title `Open settings` can miss. The item's search terms include `settings` but not `open`. Type `settings` until the row titled `Open settings` remains, then activate it. Settings General appears. The palette closes.
- **Reopen.** Press `mod+k` again from General or after `Back`. The root palette returns.
- **Add project.** Type `Add project` and activate it. With one connected environment, the palette stays open on Sources (`Local folder`). Combobox name on Sources is `Search...`. After `Local folder`, the combobox is named `Enter path (e.g. ~/projects/my-app)` and is prefilled with the server's configured base directory or `~/` when none is configured. One Escape closes the palette from any add-project view. Do not add a folder unless you are also running the landing recipe. With zero environment entries, the action closes the palette and opens `/settings/connections`.
...
- `Add project` stays enabled when no environment is connected. With zero environment entries it opens `/settings/connections`; with disconnected entries it shows an `Environments` group whose unavailable rows are disabled. After a correct isolated pair, the single connected environment goes directly to Sources. Unpaired, `mod+k` does nothing because the palette is not mounted.
- Typing the visible title `Open settings` can miss. The search haystack is `searchTerms`, which includes `settings` but not `open`. Pick the empty-query Actions row or type `settings`.
- With more than one environment, or connected and disconnected entries but no connected default, `Add project` shows an `Environments` group before `Sources`.
