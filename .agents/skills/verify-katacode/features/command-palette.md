...
- **Add project.** Type `Add project` and activate it. The palette stays open on Sources (`Local folder`). After `Local folder`, the path placeholder is `Enter path (e.g. ~/projects/my-app)`. Press Escape until the palette is gone. Do not add a folder unless you are also running the landing recipe.
