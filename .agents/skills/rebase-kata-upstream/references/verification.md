...
On a push to `main`, GitHub Check sets `BASE_SHA` to `github.sha`, so the checker compares the merge commit to itself. That receipt does not prove freeze preservation. Archive the checker from the pre-merge parent (the last PR base) and run `--mode ci` and `--mode human-review` against the landed SHA. GitHub Check may waive a single `KataUpstreamUpgrade.test.ts` trusted-assertion FAIL ([KAT-3340](https://linear.app/kata-sh/issue/KAT-3340/unfreeze-kataupstreamupgradetestts-trusted-assertion)). `human-review` has no waiver. Keep `HUMAN_REVIEW_ACCEPTANCE FAIL` when Icon Composer or other mandatory live checks are `NOT RUN`. Do not invent `PASS`.
