...
describe.skipIf(HostProcessPlatform.defaultValue() !== "linux")("installer terminal", () => {
  it.each([false, true])(
    ...
    async (fail) => {
      const root = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "katacode-install-progress-"));
      const version = "1.2.3";
      const stem = `katacode-${version}-linux-${HostProcessArchitecture.defaultValue()}`;
      ...
      await NodeFSP.writeFile(
        NodePath.join(root, stem, "katacode"),
        "#!/bin/sh\necho 'katacode v1.2.3'\n",
        {
          mode: 0o755,
        },
      );
      ...
      const child = NodeChildProcess.spawn("script", ["-qec", `sh '${installer}'`, "/dev/null"], {
        env: {
          ...
          TERM: "xterm",
          NO_COLOR: "1",
          KATACODE_VERSION: version,
          KATACODE_HOME: NodePath.join(root, "home"),
          KATACODE_INSTALL_BIN_DIR: NodePath.join(root, "bin"),
          KATACODE_RELEASE_BASE_URL: `http://127.0.0.1:${address.port}`,
        },
      ...
      try {
        ...
        if (fail) {
          ...
          expect(output).not.toContain("Installed Kata Code");
          ...
          expect(output).toContain("Installed Kata Code 1.2.3");
          expect(
            await NodeFSP.readFile(NodePath.join(versions, version, ".install-complete"), "utf8"),
          ).toBe("1.2.3\n");
          expect(
            NodeChildProcess.execFileSync(NodePath.join(root, "bin/katacode"), ["--version"], {
              encoding: "utf8",
            }).trim(),
          ).toBe("katacode v1.2.3");
