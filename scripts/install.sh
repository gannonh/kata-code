...
if [ -f "${target_dir}/.install-complete" ] && [ "$(cat "${target_dir}/.install-complete")" = "$version" ]; then
  ...
  printf '  %sInstalling%s Kata Code %s%s%s\n\n' "$muted" "$reset" "$bold" "$version" "$reset" >&2
  step "Downloading..."
  fetch_status=0
...
if "$interactive"; then printf '\r\033[2K' >&2; fi
printf '  %sInstalled Kata Code %s%s\n\n' "$green" "$version" "$reset" >&2
case ":${PATH}:" in
  *":${bin_dir}:"*) printf '  Run %skatacode%s to get started.\n\n' "$bold" "$reset" ;;
  *) printf '  Add %s to your PATH, then run %skatacode%s.\n\n' "$bin_dir" "$bold" "$reset" ;;
esac
