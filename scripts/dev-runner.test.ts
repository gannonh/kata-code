...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("createDevRunnerEnv", () => {
    it.effect("forwards the reusable auth token to web dev and removes it for desktop", () =>
      Effect.gen(function* () {
        const input = {
          baseEnv: { KATACODE_DEV_AUTH_TOKEN: "reusable-dev-auth-token-that-is-long-enough" },
          serverOffset: 0,
          webOffset: 0,
          t3Home: undefined,
          browser: undefined,
        ...
        assert.equal(web.KATACODE_DEV_AUTH_TOKEN, input.baseEnv.KATACODE_DEV_AUTH_TOKEN);
        assert.equal(desktop.KATACODE_DEV_AUTH_TOKEN, undefined);
      }),
    );
