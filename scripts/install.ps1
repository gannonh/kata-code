# Installs the Kata Code CLI from a GitHub Release archive on Windows. Needs
...
#   KATACODE_INSTALL_BIN_DIR   where katacode.exe is linked (default: ~\.local\bin)
...
$baseUrl = if ($env:KATACODE_RELEASE_BASE_URL) { $env:KATACODE_RELEASE_BASE_URL.TrimEnd("/") } else { "https://github.com/$repo/releases/download" }
$t3Home = if ($env:KATACODE_HOME) { $env:KATACODE_HOME } else { Join-Path $HOME ".katacode" }
$binDir = if ($env:KATACODE_INSTALL_BIN_DIR) { $env:KATACODE_INSTALL_BIN_DIR } else { Join-Path $HOME ".local\bin" }

function Fail([string] $message) {
  Write-Error "katacode install: $message"
  exit 1
}
...
if ($interactive) {
  ...
  for ($i = 0; $i -lt $mark.Length; $i++) {
    ...
    $label = if ($i -eq 1) { "     ${bold}Kata Code$reset" } elseif ($i -eq 2) { "     ${muted}CLI installer$reset" } else { "" }
...
$channel = if ($env:KATACODE_CHANNEL) { $env:KATACODE_CHANNEL } else { "stable" }
$version = $env:KATACODE_VERSION
if (-not $version) {
  ...
  $tagPattern = switch ($channel) {
    ...
    default { Fail "KATACODE_CHANNEL must be stable, nightly, or preview" }
  }
  $releases = Invoke-RestMethod -Uri "https://api.github.com/repos/$repo/releases?per_page=100" -Headers @{ "User-Agent" = "katacode-install" }
  $tag = ($releases | Where-Object { -not $_.draft -and $_.tag_name -match $tagPattern } | Select-Object -First 1).tag_name
  if (-not $tag) { Fail "could not find a $channel release; set KATACODE_VERSION" }
  $version = $tag.Substring(1)
}
if ($version -match '-preview\.') {
  Write-Warning "katacode $version is a preview build. Preview builds are cut by maintainers from unreleased branches to exercise the release pipeline. They can be broken, receive no fixes, and are never offered as updates. Set KATACODE_CHANNEL=stable (the default) for a supported build."
  if ($channel -ne "preview" -and -not $env:KATACODE_VERSION) {
    Fail "refusing a preview build that was not explicitly requested"
  }
}

$stem = "katacode-$version-win32-$arch"
$archive = "$stem.zip"
...
if ((Test-Path $marker) -and ((Get-Content $marker -Raw).Trim() -eq $version)) {
  ...
  try {
    ...
    [Console]::Error.WriteLine("  ${muted}Installing$reset Kata Code $bold$version$reset`n")
    Step "Downloading..."
    try {
      ...
      if ($status -eq 404) {
        Fail "katacode $version has no release archive for win32-$arch; install that version with 'npm install -g @kata-sh/code-cli@$version'"
      }
      throw
    }
    ...
    & (Join-Path $staging "katacode.exe") --version | Out-Null
...
Step "Setting up the katacode command..."
New-Item -ItemType Directory -Force -Path $binDir | Out-Null
$shim = Join-Path $binDir "katacode.cmd"
# UTF-8 without a BOM: cmd.exe reads the shim as-is, and ASCII would corrupt
# non-ASCII characters in the user's home path.
[System.IO.File]::WriteAllText($shim, "@echo off`r`n`"$(Join-Path $targetDir 'katacode.exe')`" %*", (New-Object System.Text.UTF8Encoding $false))
if ($interactive) { [Console]::Error.Write("`r$esc[2K") }
[Console]::Error.WriteLine("  ${green}Installed Kata Code $version$reset`n")
if (($env:PATH -split ";") -notcontains $binDir) {
  Write-Host "  Add $binDir to your PATH, then run ${bold}katacode$reset.`n"
} else {
  Write-Host "  Run ${bold}katacode$reset to get started.`n"
}
