...
if ((Test-Path $marker) -and ((Get-Content $marker -Raw).Trim() -eq $version)) {
  Write-Host "katacode $version is already installed at $targetDir"
} else {
...
Write-Host "Installed katacode $version"
Write-Host "  $shim -> $(Join-Path $targetDir 'katacode.exe')"
if (($env:PATH -split ";") -notcontains $binDir) {
  Write-Host "Add $binDir to your PATH to run katacode."
}
