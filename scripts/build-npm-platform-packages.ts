 ...
 * behind `npx @kata-sh/code-cli` / `npm i -g @kata-sh/code-cli`: one
 * `@kata-sh/code-cli-<platformKey>` package per archive holding the archive's
 * contents verbatim, plus the `@kata-sh/code-cli` launcher
 ...
 *
 *   @kata-sh/code-cli-<platformKey>/      archive contents + package.json
 *   @kata-sh/code-cli-<platformKey>.tgz   the same tree as an npm tarball
 *   @kata-sh/code-cli/                    launcher package
 *   @kata-sh/code-cli.tgz                 launcher tarball
 *
 ...
 */
import { legacyCliLauncherScript } from "@kata-sh/code-shared/legacyCliLauncher";
...
import { isCommandAvailable } from "@kata-sh/code-shared/shell";
import serverPackageJson from "../apps/server/package.json" with { type: "json" };

import { windowsSystemTar } from "./build-cli-archive.ts";

export const NPM_PLATFORM_PACKAGE_SCOPE = "@kata-sh";
export const NPM_LAUNCHER_PACKAGE_NAME = "@kata-sh/code-cli";
...
export function npmPlatformPackageName(platformKey: CliArchivePlatformKey): string {
  return `${NPM_PLATFORM_PACKAGE_SCOPE}/code-cli-${platformKey}`;
}
...
export function npmPlatformPackageManifest(
  ...
  return {
    ...
    version,
    description: `Kata Code CLI executable for ${platformKey}`,
    license: serverPackageJson.license,
    repository: serverPackageJson.repository,
    os: [os],
    cpu: [cpu],
    files: ["katacode", "katacode.exe", "client", "resource-monitor", "node_modules"],
...
export function npmPlatformPackageReadme(platformKey: CliArchivePlatformKey): string {
  return [
    ...
    "",
    `The Kata Code CLI executable for ${platformKey}. Do not install this package directly:`,
    ...
    "```",
    "",
    "Source and documentation: https://github.com/gannonh/kata-code",
    "",
  ].join("\n");
}

/** package.json for the CLI launcher. No engines: bin/katacode.js is trivial CJS. */
export function npmLauncherPackageManifest(
  ...
  return {
    ...
    version,
    description: "Kata Code CLI. Installs the self-contained executable for this platform.",
    license: serverPackageJson.license,
    repository: serverPackageJson.repository,
    bin: { katacode: "./bin/katacode.js" },
    files: ["bin", "dist"],
...
/**
 * The launcher every `npx @kata-sh/code-cli` runs. Plain CommonJS with no dependencies so it
...
let packageDir;
try {
  packageDir = dirname(require.resolve("${NPM_PLATFORM_PACKAGE_SCOPE}/code-cli-" + key + "/package.json"));
} catch {
  process.stderr.write(
    [
      "katacode: no Kata Code CLI build is available for this platform (" + key + ").",
      "Supported platforms: " + SUPPORTED.join(", ") + ".",
      "If yours is listed, reinstall @kata-sh/code-cli so npm fetches its optional dependency.",
      "The desktop app and release archives are at https://github.com/gannonh/kata-code/releases",
      "",
    ].join("\\n"),
  );
  process.exit(1);
...
const executable = join(packageDir, process.platform === "win32" ? "katacode.exe" : "katacode");
const result = spawnSync(executable, process.argv.slice(2), { stdio: "inherit" });
if (result.error) {
  process.stderr.write("katacode: failed to start " + executable + ": " + result.error.message + "\\n");
  process.exit(1);
}
...
const stagePlatformPackage = Effect.fn("stagePlatformPackage")(function* (input: {
  ...
  const executableName = input.key.startsWith("win32") ? "katacode.exe" : "katacode";
...
/** Writes the launcher package (package.json, bin/katacode.js, README) and its tarball. */
const stageLauncherPackage = Effect.fn("stageLauncherPackage")(function* (input: {
  ...
  const launcherScript = path.join(stageDir, "bin/katacode.js");
...
const command = Command.make(
  "build-npm-platform-packages",
  {
    archivesDir: Flag.string("archives-dir").pipe(
      Flag.withDescription("Directory holding the release's katacode archives."),
    ),
  ...
  Command.withDescription("Build the @kata-sh/code-cli npm packages from CLI release archives."),
);
