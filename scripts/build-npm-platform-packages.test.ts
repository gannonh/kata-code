...
const makeFakeArchives = Effect.fn("test.makeFakeArchives")(function* () {
  ...
  for (const key of KEYS) {
    ...
    yield* fs.writeFileString(
      path.join(contentDir, "katacode"),
      `#!/bin/sh\necho "stub ${key} $*"\nexit 7\n`,
    );
    yield* fs.chmod(path.join(contentDir, "katacode"), 0o755);
  ...
  it.effect("builds platform packages and a launcher that execs the installed one", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        ...
        ["@kata-sh/code-cli-darwin-arm64", "@kata-sh/code-cli-linux-x64", "@kata-sh/code-cli"],
      );
      for (const output of outputs) {
        assert.isTrue(yield* fs.exists(output.tarball), output.tarball);
      }

      const linuxDir = path.join(fixture.outputDir, "@kata-sh/code-cli-linux-x64");
      const linuxManifest = yield* decodeManifest(
        yield* fs.readFileString(path.join(linuxDir, "package.json")),
      );
      assert.equal(linuxManifest.name, "@kata-sh/code-cli-linux-x64");
      ...
      assert.equal(Number((yield* fs.stat(path.join(linuxDir, "katacode"))).mode) & 0o111, 0o111);

      const darwinManifest = yield* decodeManifest(
        yield* fs.readFileString(
          path.join(fixture.outputDir, "@kata-sh/code-cli-darwin-arm64/package.json"),
        ),
      );
      assert.deepStrictEqual(darwinManifest.os, ["darwin"]);
      assert.deepStrictEqual(darwinManifest.cpu, ["arm64"]);

      const launcherDir = path.join(fixture.outputDir, "@kata-sh/code-cli");
      const launcherManifest = yield* decodeManifest(
        yield* fs.readFileString(path.join(launcherDir, "package.json")),
      );
      assert.equal(launcherManifest.name, "@kata-sh/code-cli");
      assert.equal(launcherManifest.version, VERSION);
      assert.deepStrictEqual(launcherManifest.bin, { katacode: "./bin/katacode.js" });
      ...
      assert.isTrue(yield* fs.exists(path.join(launcherDir, "bin/katacode.js")));
      ...
      assert.deepStrictEqual(outputEntries.sort(), ["@kata-sh"]);
      ...
      const listing = yield* run(
        "tar",
        ["-tzvf", path.join(fixture.outputDir, "@kata-sh/code-cli-linux-x64.tgz")],
      ...
      assert.isTrue(
        lines.some((line) => /^-rwxr-xr-x .* package\/katacode$/.test(line)),
        listing.stdout,
      );
      ...
      if (KEYS.some((key) => key === `${hostPlatform}-${hostArch}`)) {
        const passthrough = yield* run(
          process.execPath,
          ["bin/katacode.js", "serve", "--port", "1234"],
          { cwd: launcherDir, env },
        );
        assert.equal(
        ...
        const unpack = yield* run(
          "tar",
          ["-xf", path.join(fixture.outputDir, "@kata-sh/code-cli.tgz"), "-C", installedLauncher],
          {
            cwd: fixture.root,
          },
        );
      ...
      const unsupported = yield* run(process.execPath, ["bin/katacode.js", "--version"], {
        cwd: launcherDir,
