...
export const DESKTOP_FILE_EXCLUSIONS = [
  // Kata Code always passes the user's installed Claude executable to the SDK,
...
interface ReleaseWslRuntimeInput {
  readonly platform: typeof BuildPlatform.Type;
  readonly runtimeArchivePath: string | undefined;
}

interface StagedWslRuntimeInput {
  readonly arch: typeof BuildArch.Type;
  readonly prebuildPath: string | undefined;
}

export function bundlesWslRuntime(input: ReleaseWslRuntimeInput): boolean;
export function bundlesWslRuntime(input: StagedWslRuntimeInput): boolean;
export function bundlesWslRuntime(input: ReleaseWslRuntimeInput | StagedWslRuntimeInput): boolean {
  if ("runtimeArchivePath" in input) {
    return input.platform === "win" && input.runtimeArchivePath !== undefined;
  }

  return input.prebuildPath !== undefined && resolveWslPrebuildArch(input.arch) !== undefined;
}
...
export class InvalidAppleTeamIdError extends Schema.TaggedError<InvalidAppleTeamIdError>()(
  ...
  override get message(): string {
    return `KATACODE_APPLE_TEAM_ID '${this.teamId}' must be a 10-character Apple Developer Team ID.`;
  }
}
...
export class MissingMacPasskeyProvisioningProfileError extends Schema.TaggedError<MissingMacPasskeyProvisioningProfileError>()(
  ...
  override get message(): string {
    return "KATACODE_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.";
  }
}
...
export class MissingMacPasskeyDomainConfigurationError extends Schema.TaggedError<MissingMacPasskeyDomainConfigurationError>()(
  ...
  override get message(): string {
    return "KATACODE_CLERK_PUBLISHABLE_KEY or KATACODE_CLERK_PASSKEY_RP_DOMAINS is required for signed macOS passkey builds.";
  }
}
...
export class InvalidMacPasskeyPublishableKeyError extends Schema.TaggedError<InvalidMacPasskeyPublishableKeyError>()(
  ...
  override get message(): string {
    return "KATACODE_CLERK_PUBLISHABLE_KEY is invalid.";
  }
}
...
export function resolveMacPasskeySigningConfiguration(
  ...
  const teamId = env.KATACODE_APPLE_TEAM_ID?.trim().toUpperCase() ?? "";
  if (!APPLE_TEAM_ID_PATTERN.test(teamId)) {
    throw new InvalidAppleTeamIdError({ teamId });
  }

  const provisioningProfilePath = env.KATACODE_MACOS_PROVISIONING_PROFILE?.trim() ?? "";
  if (provisioningProfilePath.length === 0) {
    throw new MissingMacPasskeyProvisioningProfileError();
  }

  const configuredRpDomains = env.KATACODE_CLERK_PASSKEY_RP_DOMAINS?.trim();
  let rpDomains: readonly string[];
  if (configuredRpDomains) {
    rpDomains = configuredRpDomains.split(",").map(normalizePasskeyRpDomain);
  } else {
    const publishableKey = env.KATACODE_CLERK_PUBLISHABLE_KEY?.trim();
    if (!publishableKey) {
...
export function resolveMergedStageDependencies(input: {
  ...
  if (input.platform === "mac") {
    return resolveMacStageDependencies(input);
  }
...
const BuildEnvConfig = Config.all({
  ...
  // Path to the Linux CLI release archive (katacode-<version>-linux-x64.tar.gz) built
  // by the build_linux_cli CI job. The Windows build embeds it verbatim as the
  // WSL runtime.
  wslRuntime: Config.String("KATACODE_DESKTOP_WSL_RUNTIME").pipe(Config.option),
});
...
export const resolveBuildOptions = Effect.fn("resolveBuildOptions")(function* (
  ...
  const wslRuntime =
    Option.getOrUndefined(input.wslRuntime ?? Option.none()) ??
    Option.getOrUndefined(env.wslRuntime);
...
export const preflightLinuxDesktopBuild = Effect.fn("preflightLinuxDesktopBuild")(function* (
  ...
  const reuseResourceMonitor = yield* Config.Boolean(
    "KATACODE_DESKTOP_REUSE_RESOURCE_MONITOR",
  ).pipe(Config.withDefault(false));
  const reuseCaptureHelpers = yield* Config.Boolean(
    "KATACODE_DESKTOP_REUSE_LINUX_CAPTURE_HELPERS",
  ).pipe(Config.withDefault(false));
...
export const stageLinuxCaptureHelper = Effect.fn("stageLinuxCaptureHelper")(function* (input: {
  ...
  const reuseHelpers = yield* Config.Boolean("KATACODE_DESKTOP_REUSE_LINUX_CAPTURE_HELPERS").pipe(
...
export function resolveDesktopMainProcessRuntimeDependencies(
  dependencies: Record<string, string> | undefined,
  catalog: Record<string, string>,
): Record<string, string> {
  if (!dependencies || Object.keys(dependencies).length === 0) {
...
export function resolveDesktopProductName(version: string): string {
  return resolveDesktopUpdateChannel(version) === "nightly"
    ? "Kata Code (Nightly)"
    : (desktopPackageJson.productName ?? "Kata Code");
}
    ...
    buildConfig.mac = {
      ...
      extendInfo: {
        NSScreenCaptureUsageDescription:
          "Kata Code captures the active window when you use the window capture shortcut.",
      },
...
interface CopyWslRuntimeArchiveInput {
  readonly sourceArchivePath: string;
  readonly archivePath: string;
  readonly hashPath: string;
}

interface PackWslRuntimeArchiveInput {
...
export const stageWslRuntimeArchive = Effect.fn("stageWslRuntimeArchive")(function* (
  input: CopyWslRuntimeArchiveInput | PackWslRuntimeArchiveInput,
) {
  const fs = yield* FileSystem.FileSystem;
  const path = yield* Path.Path;
  yield* fs.makeDirectory(path.dirname(input.archivePath), { recursive: true });

  if ("sourceArchivePath" in input) {
  ...
  yield* Effect.log(`[desktop-artifact] Staged WSL runtime at ${input.archivePath} (${digest}).`);
});
...
export const validateWindowsPackagedPayload = Effect.fn(
  ...
  // directory is named katacode-<version>-linux-<arch>.
  readonly appVersion?: string;
  ...
  if (hasWslArchive && hasWslArchiveHash) {
    ...
    if (input.appVersion === undefined) {
      ...
      // desktop app's WSL install script relies on that layout to find `katacode`.
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const resolvedDesktopRuntimeDependencies = yield* Effect.try({
    try: () =>
      resolveDesktopMainProcessRuntimeDependencies(
  ...
  const configuredMacPasskeySigning =
    options.platform === "mac" && options.signed
      ? yield* Effect.try({
          try: () => resolveOptionalMacPasskeySigningConfiguration(loadRepoEnv()),
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  ...
  wslRuntime: Flag.String("wsl-runtime").pipe(
    Flag.withDescription(
      "Path to the Linux CLI release archive (katacode-<version>-linux-x64.tar.gz) to embed as the WSL runtime of a Windows build (env: KATACODE_DESKTOP_WSL_RUNTIME).",
    ),
    Flag.optional,
  ),
}).pipe(
  Command.withDescription("Build a desktop artifact for Kata Code."),
