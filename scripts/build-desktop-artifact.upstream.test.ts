    ...
    const sourceArchivePath =
      input.wslRuntime === "loose-server-tree"
        ? // The old hand-rolled runtime: apps/server/dist + node_modules at the
          // archive root, no single stem directory, no `katacode` executable.
          yield* makeLinuxCliArchiveFixture({
            root: path.join(tempDir, "wsl-runtime"),
            stem: "apps",
            omitMembers: ["apps/katacode", "apps/client/index.html"],
...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      const targetedLinux = yield* createBuildConfig(
        "linux",
        "AppImage",
        "1.2.3",
        false,
      ...
      assert.deepStrictEqual(mac.dmg, {
        title: "Kata Code (Alpha) 1.2.3 Installer",
      ...
      Effect.gen(function* () {
        ...
        yield* stageResourceMonitor({
          ...
          Effect.provide(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: { KATACODE_DESKTOP_REUSE_RESOURCE_MONITOR: "true" },
              }),
            ),
          ),
        );
              ...
              ConfigProvider.layer(
                ConfigProvider.fromEnv({
                  env: {
                    ...
                    KATACODE_DESKTOP_REUSE_RESOURCE_MONITOR: "true",
                  },
                }),
              ),
            ),
  ...
  it("derives macOS passkey signing configuration from the Clerk publishable key", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      ...
      KATACODE_MACOS_PROVISIONING_PROFILE: "/tmp/t3code.provisionprofile",
      KATACODE_CLERK_PUBLISHABLE_KEY: `pk_test_${btoa("example.clerk.accounts.dev$")}`,
    });
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      KATACODE_APPLE_TEAM_ID: "ABC1234567",
      KATACODE_MACOS_PROVISIONING_PROFILE: "/tmp/t3code.provisionprofile",
      KATACODE_CLERK_PASSKEY_RP_DOMAINS:
    ...
    assert.include(entitlements, "<string>ABC1234567.com.katacode.app</string>");
  ...
  it("rejects incomplete macOS passkey signing configuration", () => {
    ...
    assert.equal(
      ...
      "KATACODE_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.",
    );
    ...
    const invalidDomainError = captureError({
      KATACODE_APPLE_TEAM_ID: "ABC1234567",
      KATACODE_MACOS_PROVISIONING_PROFILE: "/tmp/t3code.provisionprofile",
      KATACODE_CLERK_PASSKEY_RP_DOMAINS: unsafeDomain,
    });
    ...
    assert.throws(
      () =>
        resolveMacPasskeySigningConfiguration({
          KATACODE_APPLE_TEAM_ID: "ABC1234567",
          KATACODE_MACOS_PROVISIONING_PROFILE: "/tmp/t3code.provisionprofile",
          KATACODE_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev:8443",
        }),
    ...
    const invalidPublishableKeyError = captureError({
      KATACODE_APPLE_TEAM_ID: "ABC1234567",
      KATACODE_MACOS_PROVISIONING_PROFILE: "/tmp/t3code.provisionprofile",
      KATACODE_CLERK_PUBLISHABLE_KEY: "pk_test_%",
    });
    ...
    assert.equal(invalidPublishableKeyError.message, "KATACODE_CLERK_PUBLISHABLE_KEY is invalid.");
  ...
  it("ships the Linux CLI release archive as the WSL runtime", () => {
    ...
    const runtimeArchivePath = "/tmp/katacode-1.2.3-linux-x64.tar.gz";
    ...
    assert.equal(wslRuntimeArchiveStem("1.2.3", "x64"), "katacode-1.2.3-linux-x64");
  });

  it("parses Windows bsdtar member listings with CRLF line endings", () => {
    assert.deepStrictEqual(
      parseWslRuntimeArchiveMembers(
        "./katacode-1.2.3-linux-x64/katacode\r\nkatacode-1.2.3-linux-x64/client/\r\n",
      ),
      ["katacode-1.2.3-linux-x64/katacode", "katacode-1.2.3-linux-x64/client"],
    );
  });
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const error = yield* stageWslRuntimeArchive({
          sourceArchivePath: path.join(root, "katacode-1.2.3-linux-x64.tar.gz"),
