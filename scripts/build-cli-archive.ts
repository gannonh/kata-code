 ...
 * archive: the `katacode` binary, the web client, the resource monitor, and a
 ...
 * archive stem):
 *
 *   katacode | katacode.exe  the single-executable
...
export function cliArchiveStem(version: string, platform: BuildPlatform, arch: BuildArch): string {
  return `katacode-${version}-${cliArchivePlatformKey(platform, arch)}`;
}
...
const MacSigningConfig = Config.all({
  identity: Config.string("KATACODE_CLI_MAC_SIGN_IDENTITY").pipe(Config.option),
...
const signMacArchiveContents = Effect.fn("signMacArchiveContents")(function* (input: {
  ...
  if (identity === "-") {
    yield* Effect.log("[cli-archive] Signed ad hoc (no KATACODE_CLI_MAC_SIGN_IDENTITY).");
    return;
  }
  ...
  const notarizeZip = path.join(path.dirname(input.executablePath), ".notarize-katacode.zip");
  yield* runCommand(
  ...
  yield* Effect.log("[cli-archive] Notarized katacode.");
});
...
/** Signs katacode.exe through the same Azure Trusted Signing setup the installer uses. */
const signWindowsExecutable = Effect.fn("signWindowsExecutable")(function* (
  ...
  yield* runCommand(
    ...
    "Invoke-TrustedSigning katacode.exe",
  );
  yield* Effect.log("[cli-archive] Signed katacode.exe (Azure Trusted Signing).");
});
...
const buildCliArchive = Effect.fn("buildCliArchive")(function* (input: {
  ...
  const executableName = input.platform === "win" ? "katacode.exe" : "katacode";
  // tsdown suffixes cross-built executables with their target (katacode-darwin-x64);
  // a host build is plain katacode. Prefer the exact target when both exist.
  const targetKey = `${input.platform === "mac" ? "darwin" : input.platform}-${input.arch}`;
  const targetExecutable = path.join(
    serverDir,
    "dist-exe",
    `katacode-${targetKey}${input.platform === "win" ? ".exe" : ""}`,
  );
  ...
  yield* requireInput(
    ...
    "Run `vp run --filter @kata-sh/code-cli build` first.",
  );
  yield* requireInput(
    resourceMonitorDir,
  ...
  const stageRoot = yield* fs.makeTempDirectoryScoped({ prefix: "katacode-cli-archive-" });
...
const command = Command.make(
  ...
  Command.withDescription("Package the katacode single-executable into a per-platform archive."),
);
