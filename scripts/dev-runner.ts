...
/**
 * `role` matters because only the backend honours `--host`/`KATACODE_HOST`; the
...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedErrorClass<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("KATACODE_PORT_OFFSET"),
...
> {
    ...
    if (config.portOffset < 0) {
      return Effect.fail(
        new DevRunnerInvalidPortOffsetError({
          configKey: "KATACODE_PORT_OFFSET",
  ...
  if (seed) {
    if (/^\d+$/.test(seed)) {
      return Effect.succeed({
        offset: Number(seed),
        source: `numeric KATACODE_DEV_INSTANCE=${seed}`,
      });
    }

    const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
    return Effect.succeed({ offset, source: `hashed KATACODE_DEV_INSTANCE=${seed}` });
  }
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .katacode > ambient KATACODE_HOME) is resolved
    ...
    if (configuredBaseDir !== undefined) {
      output.KATACODE_HOME = resolvedBaseDir;
    } else {
      delete output.KATACODE_HOME;
    }
    ...
    if (!isDesktopMode) {
      ...
      if (mode === "dev" || mode === "dev:web") {
        ...
        delete output.KATACODE_SINGLE_ORIGIN_DEV;
      }
    } else {
      output.KATACODE_PORT = String(serverPort);
    ...
    if (!isDesktopMode) {
      output.KATACODE_NO_BROWSER = browser === true ? "0" : "1";
    }
 ...
 * target reach it there. When `--host`/`KATACODE_HOST` moves the backend onto
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["KATACODE_PORT_OFFSET", "KATACODE_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
    ...
    // (gitignored) `.katacode` — see @kata-sh/code-shared/devHome for why this must
    // outrank an ambient KATACODE_HOME. `--home-dir` still wins.
    const worktreeHome = yield* resolveWorktreeKatacodeHome(yield* HostProcessWorkingDirectory);
    ...
    const resolvedT3Home =
      ...
      worktreeHome ??
      (hostEnvironment.KATACODE_HOME?.trim() || undefined);
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.KATACODE_HOME ?? (yield* DEFAULT_KATACODE_HOME);

    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.KATACODE_PORT)} webPort=${String(env.PORT)} baseDir=${baseDir}`,
    );
    ...
    if (input.share) {
      if (input.mode === "dev:server") {
        ...
        if (shared) {
          ...
          env.KATACODE_DEV_ALLOWED_ORIGINS = [
            env.KATACODE_DEV_ALLOWED_ORIGINS,
          ...
          // a few chunk requests. Only defaulted, so KATACODE_BUNDLED_DEV=0
          // still opts a --share run back out.
          if (env.KATACODE_BUNDLED_DEV === undefined) {
            env.KATACODE_BUNDLED_DEV = "1";
          }
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  t3Home: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit Kata Code data directory; runtime state is stored under userdata (equivalent to KATACODE_HOME). Inside a git worktree this defaults to that worktree's own .katacode so dev state stays off the shared home.",
    ),
    Flag.optional,
